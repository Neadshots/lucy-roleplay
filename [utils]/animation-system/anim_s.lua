...
function coverAnimation(thePlayer)
	...
	if (logged==1) and (getElementData(thePlayer, "dead") ~= 1) then
...
function kissingAnimation( thePlayer, commandName, target )
	...
	if not(logged==1) and (getElementData(thePlayer, "dead") ~= 1) then
		return false
	end	
...
function ysexAnimation( thePlayer, commandName, target )
	...
	local tx, ty, tz = getElementPosition(targetPlayer)
	if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<1) and getPedOccupiedVehicle(thePlayer) == false and getPedOccupiedVehicle(targetPlayer) == false then
			exports.global:applyAnimation( thePlayer, "BLOWJOBZ", "BJ_Couch_End_P", -1, false, false, false )
		exports.global:applyAnimation( targetPlayer, "BLOWJOBZ", "BJ_Couch_End_W", -1, false, false, false )
	end
end
...
function ysex2Animation( thePlayer, commandName, target )
	...
	local tx, ty, tz = getElementPosition(targetPlayer)
	if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<1) and getPedOccupiedVehicle(thePlayer) == false and getPedOccupiedVehicle(targetPlayer) == false then
			exports.global:applyAnimation( thePlayer, "BLOWJOBZ", "BJ_Couch_Loop_P", -1, false, false, false )
		exports.global:applyAnimation( targetPlayer, "BLOWJOBZ", "BJ_Couch_Loop_W", -1, false, false, false )
	end
end
...
function giveGuitar(thePlayer, cmd, arg)
	local logged = getElementData(thePlayer, "loggedin")

	if (logged==1) then
		if isElement(playerGuitars[thePlayer]) then
			destroyElement(playerGuitars[thePlayer])
			return
		end

		triggerClientEvent(thePlayer,"Animasyon:OzelAnim",thePlayer,nil,"guitar","car_hookertalk","Anim")
		--exports.global:applyAnimation( thePlayer, "dance", "weapon_crouch", -1, false, false, false)
		playerGuitars[thePlayer] = createObject(3063, 0, 0, 0)
		setElementInterior(playerGuitars[thePlayer], getElementInterior(thePlayer))
		setElementDimension(playerGuitars[thePlayer], getElementDimension(thePlayer))
        exports.bone_attach:attachElementToBone(playerGuitars[thePlayer],thePlayer,3,0,0,0,0,-90,0) 
		triggerEvent("bindAnimationStopKey",thePlayer)
	end
end
addCommandHandler ( "gitar", giveGuitar, false, false )

addEventHandler("onPlayerQuit", root,
	function()
		if isElement(playerGuitars[source]) then
			destroyElement(playerGuitars[source])
		end
	end
)
...
addEventHandler("anim_editor.save", root,
	function(player, details)
		...
		exports.global:applyAnimation(player, details.body, details.part, -1, false, false, false)
	end
)