...
function toggleStatus(thePlayer, section, status)
	...
	exports.global:sendWrnToStaff(getPlayerName(thePlayer).." has toggled "..section.." to "..newstatus, "STATUS")
end
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if reportingPlayer == thePlayer and not exports.integration:isPlayerHeadAdmin(thePlayer) then
