...
local xmlPath = ":weapon/attachments.xml"
local weaponTable = {}
...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 200, 200, "Silah Konumlandırma", false)
	...
	local weaponAttached = guiGridListAddColumn(weaponsGridList, "Yer.", 0.2)
	addEventHandler("onClientGUIDoubleClick", weaponsGridList, doubleClickedWeapon, false)
	populateWeaponGridList()

	local help = guiCreateLabel(12, 155, 200, 10, "Çift tıklayarak aktif edebilirsiniz.", false, sWin)
...
function createEditWindow()
	...
	local sx, sy = guiGetScreenSize()
	eWin = guiCreateWindow(sx - 250, sy - 375, 225, 325, "Silah Konumlandırma Ayarları", false)
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	object = createObject(weaponModels[editingWeapon], x, y, z)
...
local renderSync = false
local syncedWeapons = {}

addEvent("weaponSync.mode", true)
addEventHandler("weaponSync.mode", root,
	function(weaponIndex, state)
		if state == false and renderSync then
			removeEventHandler("onClientRender", root, checkWeaponPosition)
			renderSync = false
			for index, value in ipairs(syncedWeapons) do
				if value == weaponIndex then
					table.remove(syncedWeapons, index)
				end
			end
			return
		end
		addEventHandler("onClientRender", root, checkWeaponPosition)
		renderSync = true
		odim, oint = getElementDimension(localPlayer), getElementInterior(localPlayer)
		syncedWeapons[#syncedWeapons + 1] = weaponIndex
	end
)

function checkWeaponPosition()
	if getElementInterior(localPlayer) ~= oint then--int check
		for index, value in ipairs(syncedWeapons) do
			triggerServerEvent("setWeaponInteriorDimension", localPlayer, value, getElementInterior(localPlayer), getElementDimension(localPlayer))
		end
		oint = getElementInterior(localPlayer)
	end

	if getElementDimension(localPlayer) ~= odim then
		for index, value in ipairs(syncedWeapons) do
			triggerServerEvent("setWeaponInteriorDimension", localPlayer, value, getElementInterior(localPlayer), getElementDimension(localPlayer))
		end
