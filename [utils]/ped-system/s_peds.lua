            ...
            if rows > 0 then
                for index, row in ipairs(res) do
                    ...
                    if ped then
                        ...
                        exports.anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdByUsername", exports.global:getUserNameFromID(row.created_by))
...
function healByHospitalPed(ped)
    ...
    if(needed < 5) then
        exports.RPPchat:chatPed(ped, "say", "You seem to be fine, sir. No treatment needed.")
    else
        ...
        if(getPlayerMoney(source) > cost or getPlayerMoney(source) == cost) then
            ...
            exports.RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to "..pedname)
            exports.RPPchat:chatPed(ped, "me", "takes the money")
            takePlayerMoney(source, tonumber(cost))
            exports.RPPchat:chatPed(ped, "say", "Thank you sir. I'll get you fixed up.")
            exports.RPPchat:chatPed(ped, "me", "heals "..playername)
            setElementHealth(source, newHealth)
        else
            exports.RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to "..pedname)
            exports.RPPchat:chatPed(ped, "say", "Sorry sir. I don't think that is enough.")
        end
    end
end
