...
function createShop ( id, x, y, z, int, dim, type )
    ...
    exports.anticheat:changeProtectedElementDataEx(shop, "id", id, false, true)
    exports.anticheat:changeProtectedElementDataEx(shop, "shop", true, false, true)
    exports.anticheat:changeProtectedElementDataEx(shop, "type", type, false, true)
end
...
function makeShop ( thePlayer, commandName, type )
    ...
    local id = SmallestID( )
    local result = dbExec(mysql:getConnection(), "INSERT INTO nshops SET id='"  .. mysql:escape_string(id) .. "', x='"  .. mysql:escape_string(x) .. "', y='" .. mysql:escape_string(y) .. "', z='" .. mysql:escape_string(z) .. "', interior='" .. mysql:escape_string(int) .. "', dimension='" .. mysql:escape_string(dim) .. "', type='" .. mysql:escape_string(type) .. "' ")
    if (result) then
        createShop ( id, x, y, z, int, dim, type )
...
function delShop( thePlayer, commandName, shopID )
    if not exports.integration:isPlayerSeniorAdmin(thePlayer) then return end
    ...
    local result = dbExec(mysql:getConnection(), "DELETE FROM nshops WHERE id='" .. mysql:escape_string(id) .. "'")
    if (result) then
...
function setShopSize ( thePlayer, commandName, id, size )
    ...
    local result = dbExec(mysql:getConnection(), "UPDATE nshops SET size='"  .. mysql:escape_string(size) .. "' WHERE id='" .. mysql:escape_string(id) .. "'")
    if (result) then
                ...
                for k, s in ipairs (shops[type]) do
                    --for k, s in ipairs (shops[i]) do
                        if itemname == s[1]:lower() then
                            if exports.global:hasMoney(thePlayer, s[2]) then
                                if s[3] == "eat" then
                                    hunger = getElementData(thePlayer, "hunger")
                                    exports.anticheat:changeProtectedElementDataEx(thePlayer, "hunger", hunger+s[4])
                                    exports.global:applyAnimation(thePlayer, "FOOD", "EAT_Burger", 2000, false, true, true)
                                    --break
