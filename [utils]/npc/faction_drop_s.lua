...
function factionDropCreateItem(npcID, itemID, itemValue, itemPrice, itemQuan, itemCaliber, itemDesc, itemRestock, selectedItem)
	...
	if not dbExec(mysql:getConnection(), "INSERT INTO `shop_products` SET `pItemID`='"..itemID.."', `pItemValue`='"..finalItemValue.."', `pPrice`='"..itemPrice.."', `pQuantity`='"..itemQuan.."', `pSetQuantity`='"..itemQuan.."', `pRestockInterval`='"..itemRestock.."', `npcID`='"..npcID.."', `pRestockedDate`=NOW()") then
...
function refreshFactionDropWeaponList(thePlayer,id)
	...
	{thePlayer}, mysql:getConnection(), "SELECT `npcID`, `pItemID`, `pItemValue`, `pDesc`, `pPrice`, `pDate`, `pID`, `pQuantity`, `pSetQuantity`, `pRestockInterval`, `pRestockedDate`, DATEDIFF((`pRestockedDate` + interval `pRestockInterval` day),NOW()) AS `pRestockIn` FROM `shop_products` WHERE `npcID`='"..id.."' ORDER BY `pID` DESC")
end
...
function factionDropWeaponBuy(proID, theShop, reasonToBuy)
	...
	dbQuery(
		function(qh, source, proID, theShop, reasonToBuy)
			local res, rows, err = dbPoll(qh, 0)
			if rows > 0 then
				for index, product in ipairs(res) do
					...
					if not dbExec(mysql:getConnection(), prepared) then
	...
	{source, proID, theShop, reasonToBuy}, mysql:getConnection(), "SELECT * FROM `shop_products` WHERE `pID`='"..proID.."' AND `npcID`='"..npcID.."' LIMIT 1")
end
...
function factionDropTakeDown(proID, theShop)
	...
	dbQuery(
		function(qh, source, proID, theShop)
			local res, rows, err = dbPoll(qh, 0)
			if rows > 0 then
				for index, product in ipairs(res) do
	...
	{source, proID, theShop}, mysql:getConnection(), "SELECT * FROM `shop_products` WHERE `pID`='"..proID.."' AND `npcID`='"..npcID.."' LIMIT 1")
end
...
function saveShopChangesToSQL()
	...
	for i, shop in pairs(shops) do
		...
		if shoptype == 18 or shoptype == 19 then
			...
			if shopID and dbExec(mysql:getConnection(), "UPDATE `shops` SET `faction_belong`='"..exports.global:toSQL(faction_belong).."', `faction_access`='"..exports.global:toSQL(faction_access).."' WHERE `id`='"..exports.global:toSQL(shopID).."' ") then
				counter = counter + 1
			else
	...
	if dbExec(mysql:getConnection(), "UPDATE `shop_products` SET `pQuantity`=`pSetQuantity`, `pRestockedDate`=NOW() WHERE `pRestockedDate` IS NOT NULL AND `pRestockInterval`>0 AND (DATEDIFF((`pRestockedDate` + interval `pRestockInterval` day),NOW())=0)") then
