	...
	if (element) and (getElementType(element)=="object") and (button=="right") and (state=="down") then
		...
		if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=maxDistance) then
			...
			if getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("item-world")) then
				...
				if (getElementDimension(element) ~= 0 or exports.integration:isPlayerTrialAdmin(localPlayer) or getElementData(localPlayer, "admin:tempmove")) and not getElementData(localPlayer, "jailed") then
					if not rcMenu then rcMenu = rightclick:create(itemName) end
			...
			if(model == 2517) then --SHOWERS
				if not rcMenu then  rcMenu = exports.rightclick:create("Duş Kabini") end
				if showering[1] then
					row.a = exports.rightclick:addrow("Duştan Çık")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				else
					row.a = exports.rightclick:addrow("Duş Al")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				end
