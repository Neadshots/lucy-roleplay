...
function ckEtme(thePlayer, command, karsi)
	...
	local karsiOyuncu = exports.global:findPlayerByPartialNick(thePlayer, karsi)
...
function onaylamaPaneli(onaylama, thePlayer, targetPlayer)
	...
	if tostring(onaylama) == "kabul" then
			if targetPlayer then
						...
						dbExec(mysql:getConnection(), "UPDATE characters SET cked='1', ck_info='Kendi istegi', `death_date`=NOW() WHERE id = " .. getElementData(thePlayer, "dbid"))--Lauren
						local x, y, z = getElementPosition(thePlayer)
						local skin = getPedSkin(thePlayer)
						local rotation = getPedRotation(thePlayer)
						--local look = getElementData(thePlayer, "look")
						--local desc = look[5]
						call( getResourceFromName( "realism" ), "addCharacterKillBody", x, y, z, rotation, skin, getElementData(thePlayer, "dbid"), targetPlayerName, getElementInterior(thePlayer), getElementDimension(thePlayer), getElementData(thePlayer, "age"), getElementData(thePlayer, "race"), getElementData(thePlayer, "weight"), getElementData(thePlayer, "height"), "yok", "Kendi istegi", getElementData(thePlayer, "gender"))
				outputChatBox("[!] #ffffffCk oldun aq cocugu.", thePlayer, 0, 0, 255, true)
