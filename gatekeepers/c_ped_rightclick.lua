		...
		if (gatekeeper) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				pedName = tostring(pedName):gsub("_", " ")
						...
						elseif (ped=="Albert Jackson") then
							triggerEvent("piyango:create",getLocalPlayer())
						...
						elseif (ped=="Andre Martin") then
							triggerServerEvent('clothing:list', getResourceRootElement (getResourceFromName("clothingstore")))
						...
						elseif (ped=="Georgio Dupont") then
       					...
       					elseif (ped=="Julie Dupont") then 
       						triggerServerEvent('clothing:list', getResourceRootElement(getResourceFromName("clothes")))
						...
						elseif (ped=="Robert Dunston") then --HP release / Maxime
							triggerServerEvent("tow:openReleaseGUI", localPlayer, ped)
						elseif (ped=="Jack Nicholas") then
							triggerServerEvent("vergi:sVergiGUIf", localPlayer, localPlayer)
						elseif (ped=="Rick Nicholas") then
							triggerServerEvent("vergi:sVergiGUI", localPlayer, localPlayer)
...
function hidePedMenu()
	...
	exports.rightclick:destroy(wPedRightClick)
	bTalkToPed = nil
