...
function toggleFreecam(cmd)
	if not isEnabled(localPlayer) and cmd == "freecam" then
		if getElementData(localPlayer, "loggedin") == 1 and (exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerScripter(localPlayer) or getElementData(localPlayer, "canFly") )then
			--removePedFromVehicle(localPlayer)
			setElementAlpha(localPlayer, 0)
			setElementFrozen(localPlayer, true)
			setFreecamEnabled (x, y, z)
			if getElementData(localPlayer,"reconx") then
				exports['admin-system']:toggleRecon(false)
			end
