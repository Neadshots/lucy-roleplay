...
BizNoteFont = guiCreateFont( "seguisb.ttf", 30 )
BizNoteFont18 = guiCreateFont( "seguisb.ttf", 18 )
...
function showGeneralshopUI(shop_type, race, gender, discount, products1)
	...
	if not wCustomShop and not wAddingItemsToShop and not wGeneralshop and not getElementData(getLocalPlayer(), "shop:NoAccess" ) then
		...
		if shop_type==17 then
			...
			wCustomShop = guiCreateWindow(X,Y,Width,Height,bizName.." - Özel Market "..version,false)
			...
			if isClientBizOwner then
				...
				guiSetText(lWelcomeText,'"Hoşgeldin patron! Naber?" || '..currentCap..'/'..sCapacity..' Ürünler , Toplam Gelir: $'..exports.global:formatMoney(sIncome)..'.')
				...
				GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Gelir: $"..exports.global:formatMoney(sIncome),false,tBizManagement)
				GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Personel Ödemesi: $"..exports.global:formatMoney(sPendingWage).." ($"..exports.global:formatMoney(sCapacity/wageRate).."/hour)",false,tBizManagement)
				GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- kâr: $"..exports.global:formatMoney(sProfit),false,tBizManagement)
				...
				if sProfit < 0 then
					...
					if sProfit < (0 - warningDebtAmount) then 
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (Uyarı: Eğer borcunuz varsa $"..exports.global:formatMoney(limitDebtAmount)..", Çalışanlarınız işten ayrılacaktır)." )
						guiLabelSetColor ( GUIEditor_Label[5], 255, 0, 0)
				...
				elseif sProfit == 0 then
					...
					if sProfit < 500 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (Ortalama).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 255, 0)
					elseif sProfit >= 500 and sProfit < 1000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (İyi!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 245, 0)
					elseif sProfit >= 1000 and sProfit < 2000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (Çok iyi!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 235, 0)
					elseif sProfit >= 2000 and sProfit < 4000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (Mükemmel!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 225, 0)
					elseif sProfit >= 4000 and sProfit < 8000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (Beklenmedik!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
					elseif sProfit >= 8000 and sProfit < 20000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (İnanılmaz!!!).")
						guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
					elseif sProfit >= 20000 then
						guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (İşin kralıyız!!!!).")
				...
				addEventHandler( "onClientGUIClick", bExpand, function ()
					...
					guiSetText(GUIEditor_Label[4] , "- Personel Ödemesi: $"..exports.global:formatMoney(sPendingWage).." ($"..exports.global:formatMoney(getElementData(ped, "sCapacity")/wageRate).."/Saatlik)")
		...
		elseif shop_type==19 then --Faction Drop NPC - Weapons
			...
			wCustomShop = guiCreateWindow(X,Y,Width,Height,"Birlik Marketi - "..version,false)
			guiWindowSetSizable(wCustomShop,false)
			...
			wGeneralshop = guiCreateWindow(X,Y,Width,Height,bizName.." - "..shop.name.." "..version,false)
			...
			for _, category in ipairs( shop ) do
				...
				local cName = guiGridListAddColumn( grid, "İsim", 0.25 )
				...
				for _, item in ipairs( category ) do
					...
					guiGridListSetItemText( grid, row, cName, exports['item-system']:getItemName(item.itemID, item.itemValue) or "-", false, false )
				...
				local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'"'..bizName..'!": Hoşgeldiniz! birşey satın almak için üzerine çift tıklayın ',true,tab)
				guiLabelSetVerticalAlign(lWelcomeText,"center")
				guiLabelSetHorizontalAlign(lWelcomeText,"center",false)
				guiSetFont(lWelcomeText, BizNoteFont18)
				if isClientBizOwner then
					guiSetText(lWelcomeText,'"Hoşgeldin patron! Naber?" || Toplam malzemeler: '..interiorSupplies..' kg(s)')
				end
				bCloseStatic[_] = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Kapat", true, tab)
			...
			if isClientBizOwner then
				...
				if shop_type ~= 14 then -- Lazy fix for non-profitable carpart shop,  maxime
					...
					GUIEditor_Label[3] = guiCreateLabel(11,95,716,20,"- Gelir: $"..exports.global:formatMoney(sIncome),false,tBizManagement)
					GUIEditor_Label[4] = guiCreateLabel(11,115,716,20,"- Personel ödemesi: $"..exports.global:formatMoney(sPendingWage).." (Already bound to interior taxes)",false,tBizManagement)
					GUIEditor_Label[5] = guiCreateLabel(11,135,716,20,"- kâr: $"..exports.global:formatMoney(sProfit),false,tBizManagement)
					GUIEditor_Label[6] = guiCreateLabel(11,155,57,19,"- Satışlar: ",false,tBizManagement)
					GUIEditor_Memo[1] = guiCreateMemo(11,179,498,184,sSales,false,tBizManagement)
					guiMemoSetReadOnly(GUIEditor_Memo[1], true)
					...
					elseif sProfit == 0 then
						...
						elseif sProfit >= 4000 and sProfit < 8000 then
							...
							guiLabelSetColor ( GUIEditor_Label[5], 0, 215, 0)
						elseif sProfit >= 8000 and sProfit < 20000 then
							guiSetText(GUIEditor_Label[5] , "- Profit: $"..exports.global:formatMoney(sProfit).." (İnanılmaz!!!).")
							guiLabelSetColor ( GUIEditor_Label[5], 0, 205, 0)
						elseif sProfit >= 20000 then
							guiSetText(GUIEditor_Label[5] , "- kâr: $"..exports.global:formatMoney(sProfit).." (İşin kralıyız!!!!).")
							guiLabelSetColor ( GUIEditor_Label[5], 0, 195, 0)
				...
				local lPhone = guiCreateLabel(11,95,716,20,"- Telefon Numarası: "..sFormatedPhone.."",false,tGoodBye)
				local lEmail = guiCreateLabel(11,115,716,20,"- Email Adresi: "..sEmail.."",false,tGoodBye)
				local lForums = guiCreateLabel(11,135,716,20,"- ((Forum İsmi: "..sForum.."))",false,tGoodBye)
				local lGuide = guiCreateLabel(0.01,0.5,1,0.1,"        'Hey, mesajını istersen patronlarıma iletebilirim': ",true,tGoodBye)
				...
				if not getElementData(getLocalPlayer(), "shop:coolDown:contact") then
					...
					guiSetText(bSend, "(Başka bir mesaj göndermek için "..coolDownSend.." Dakika beklemeniz gerekiyor)")
					guiSetEnabled(bSend, false)
				end	
				...
				addEventHandler( "onClientGUIClick", bSend, function()
					...
					if contactContent and contactContent ~= "" and contactContent ~= "content" then
						...
						guiSetText(bSend, "(Başka bir mesaj göndermek içnin "..coolDownSend.." Dakika beklemeniz gerekiyor)")
						guiSetEnabled(bSend, false)
				...
				addEventHandler( "onClientGUIAccepted", eContent,function()
					...
					if contactContent and contactContent ~= "" and contactContent ~= "content" then
						...
						guiSetText(bSend, "(Başka bir mesaj gönderebilmek için "..coolDownSend.." Dakika beklemeniz gerekiyor)")
						guiSetEnabled(bSend, false)
