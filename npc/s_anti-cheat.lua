...
addEventHandler("onResourceStart", getResourceRootElement(getThisResource()), function() 
	for i, player in pairs(getElementsByType("player")) do
		setElementData(player, "usingThisShop", false)
	end
end)
...
function setShopCurrentUser(shop, player)
	...
	if shop and getElementType(shop) == "ped" and getElementType(player) == "player" then
		setElementData(shop, "usingThisShop", player)
		setElementData(player, "usingThisShop", shop)
