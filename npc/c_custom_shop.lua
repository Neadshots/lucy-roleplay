...
function addItemToShop(source, item, slot, worldItem, npcID, shopElement )
	if tonumber(getElementData(shopElement, "currentCap")) >= tonumber(getElementData(shopElement, "sCapacity")) then
		triggerServerEvent("shop:storeKeeperSay", source, source, "Hey hey! Ücretimi yükseltmediğin sürece hiçbir şey satmıyorum.", getElementData(shopElement, "name"))
		return false
	end
	...
	wAddingItemsToShop = guiCreateWindow( X, Y, Width, Height,"Bir ürün satışa çıkarıldı",false)
	...
	if not exports["item-system"]:getItemHideItemValue(tonumber(item[1])) then
		itemValue = exports["item-system"]:getItemValue( tonumber(item[1]), tostring(item[2]) ) or ""
	end
	
	local iProductImage = guiCreateStaticImage(9,27,128,128,itemURL,false,wAddingItemsToShop)
	local lProductName = guiCreateLabel(147,27,280,18,"Ürün ismi: "..itemName or ""..".",false,wAddingItemsToShop)
	local lAmount = guiCreateLabel(147,45,280,18,"Detaylar: "..itemValue or ""..".",false,wAddingItemsToShop) 
	local lPrice = guiCreateLabel(147,63,42,18,"Fiyat: $",false,wAddingItemsToShop)
	local ePrice = guiCreateEdit(189,62,238,20,"0",false,wAddingItemsToShop)
	local lDesc = guiCreateLabel(147,81,66,18,"Hakkında: ",false,wAddingItemsToShop)
	...
	local bCancel = guiCreateButton(217,161,210,27,"Kapat",false,wAddingItemsToShop)
	addEventHandler( "onClientGUIClick", bCancel, function ()
		closeAddingItemWindow()
	end, false )
	local bOk = guiCreateButton(9,161,208,27,"Tamam, bunu satışa çıkar lütfen!",false,wAddingItemsToShop)
	addEventHandler( "onClientGUIClick", bOk, function ()
		...
		if price then
			...
			guiSetText(wAddingItemsToShop, "Geçersiz Fiyat")
			setTimer(function()
				guiSetText(wAddingItemsToShop,"Bu ürün ("..itemName..") satışta")
			end, 3000, 1)
		end
	end, false )
end
...
function ownerProductView(products, proID, shopElement)
	...
	wOwnerProductView = guiCreateWindow( X, Y, Width, Height,"Ürün yönetimi",false)
	...
	local lProductName = guiCreateLabel(147,27,280,18,"Ürün ismi: "..itemName or ""..".",false,wOwnerProductView)
	local lAmount = guiCreateLabel(147,45,280,18,"Detaylar: "..itemValue or ""..".",false,wOwnerProductView) 
	local lPrice = guiCreateLabel(147,63,42,18,"Fiyat: $",false,wOwnerProductView)
	local ePrice = guiCreateEdit(189,62,238,20,item[4] or 0,false,wOwnerProductView)
	local lDesc = guiCreateLabel(147,81,66,18,"Hakkında: ",false,wOwnerProductView)
	local mDesc = guiCreateMemo(147,99,280,56,item[3] or "",false,wOwnerProductView)
	
	local bTakeOff = guiCreateButton(9,161,142,27,"Çıkar",false,wOwnerProductView)
	...
	local bSave = guiCreateButton(151,161,142,27,"Kaydet",false,wOwnerProductView)
	addEventHandler( "onClientGUIClick", bSave, function ()
		...
		if price then
			...
			guiSetText(wOwnerProductView, "Geçersiz Fiyat!")
			setTimer(function()
				guiSetText(wOwnerProductView,"Ürün yönetimi")
			end, 3000, 1) 
		end
	end, false )
...
function customShopBuy(products, proID, shopElement)
	...
	local lProductName = guiCreateLabel(147,27,280,18,"Ürün adı: "..itemName or ""..".",false,wCustomShopBuy)
	local lAmount = guiCreateLabel(147,45,280,18,"Detaylar: "..itemValue or ""..".",false,wCustomShopBuy) 
	local lPrice = guiCreateLabel(147,63,42,18,"Fiyat: $",false,wCustomShopBuy)
	local ePrice = guiCreateLabel(189,62,238,20,exports.global:formatMoney(itemPrice),false,wCustomShopBuy)
	local lDesc = guiCreateLabel(147,81,66,18,"Hakkında: ",false,wCustomShopBuy)
	...
	local bPayByBank = guiCreateButton(9,161,142,27,"Kredi kartı ile öde",false,wCustomShopBuy)
	...
	local bPayByCash = guiCreateButton(151,161,142,27,"Nakit Ödeme",false,wCustomShopBuy)
	...
	local bCancel = guiCreateButton(291,161,136,27,"Kapat",false,wCustomShopBuy)
	addEventHandler( "onClientGUIClick", bCancel, function ()
		closeCustomShopBuy()
	end, false )
end
