...
function processOpenTolls(tollKey, thePed, thePlayer, payByBank)
	if not tollElementsLocked[tollKey] or exports.global:hasItem(thePlayer, 64) or exports.global:hasItem(thePlayer, 65) or exports.global:hasItem(thePlayer, 112) then
		if payByBank then
			...
			if not exports.global:takeMoney(thePlayer, 30) then
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if factionID ~= 1 and factionID ~= 87 and factionID ~= 59 and not exports.integration:isPlayerTrialAdmin(thePlayer) then
		return
	end
	...
	if tollElementsLocked[gateID] then
		local first = true
		exports['chat']:departmentradio(thePlayer, "d", "Please lock down the "..tollElementsName[gateID].." tolls, how copy?, over.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat']:departmentradio(thePed, "d", "10-4, locking down "..tollElementsName[gateID]..", out.")
					first = false
				end
		...
		local first = true
		exports['chat']:departmentradio(thePlayer, "d", "Please open the "..tollElementsName[gateID].." tolls, how copy?, over.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat']:departmentradio(thePed, "d", "10-4, opening up "..tollElementsName[gateID]..", out.")
					first = false
				end
