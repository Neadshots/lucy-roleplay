...
function deleteAllItemsFromAnInterior(thePlayer, commandName, intID , dayOld, restartRes)
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		if not tonumber(intID) or tonumber(intID) < 0 or tonumber(intID)%1 ~=0 or  not tonumber(dayOld) or tonumber(dayOld) < 0 or tonumber(dayOld)%1 ~=0 then
			...
			if tonumber(intID) == 0 and not exports.integration:isPlayerDeveloper( thePlayer ) then
				outputChatBox("Only Head+ Admins can delete all item instances from world map.", thePlayer, 255, 0, 0)
				return false
			end
			
			-- if tonumber(intID) == 0 and exports.integration:isPlayerDeveloper( thePlayer ) then
				-- restartRes = 1
			-- end
...
function delTacos(thePlayer, commandName)
	if exports.integration:isPlayerScripter(thePlayer) then
		deleteAll(11,1)
	end
end
addCommandHandler("deltacos", delTacos)
