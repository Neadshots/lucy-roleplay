	...
	if not ammo then
		...
		if itemID == 162 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere Body Armor atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
		...
		if itemID == 56 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere Yüz Maskesi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 134 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 254 then
			...
			--outputDebugString("Check")
			if hoursPlayed < 9999999 then
				outputChatBox("[!] #ffffffMalesef, bu eşyayı yere atamazsınız.", source, 255, 0, 0, true)
				--outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				--exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
		...
		if itemID == 116 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere mermi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 115 then
			local val = exports.global:explode(":", itemValue)
			...
			--exports.global:sendMessageToAdmins("|| Inception Roleplay || BUG Koruma > ".. getPlayerName(client) .. ", Yere silah atmaya calisti.")
			--return
			triggerClientEvent( source, "finishItemDrop", source )
		end
		...
		if itemID == 60 or itemID == 137 or itemID == 138 or itemID == 175 and not exports.integration:isPlayerAdmin(source) then
			outputChatBox( "[!] #ffffffBu eşya yere atılamaz.", source, 255, 0, 0, true)
		elseif ( itemID == 81 or itemID == 103 ) and dimension == 0 then
			outputChatBox( "[!] #ffffffBu öğeyi bir ev/işyeri içine bırakmanız gerekiyor.", source, 255, 0, 0, true)
		...
		elseif itemID == 139 then
			...
			elseif (itemID == 147) then -- Picture Frame
				local dimension = getElementDimension(source)
				if dimension > 0 or (exports.integration:isPlayerDeveloper(source) and exports.global:isAdminOnDuty(source)) or exports.integration:isPlayerScripter(source) then
			...
			if insert then
				...
				if itemID == 134 then
					outputChatBox("" .. exports.global:formatMoney(itemValue) .. "$ yere bırakır.", source, 255, 194, 14)
				...
				if itemID == 134 then
					triggerEvent('sendAme', source, "" .. exports.global:formatMoney(itemValue) .. "$ bırakır.")
		...
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("[!] #ffffffSilahlarınızı görev başındayken bırakamazsınız.", source, 255, 0, 0, true)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("[!] #ffffffBu sprey kutusunu bırakamazsın.", source, 255, 0, 0, true)
		else
...
addEventHandler('item:move:save', root,
	function(x, y, z, rx, ry, rz)
		...
		if exports.global:isStaffOnDuty(client) or exports.integration:isPlayerDeveloper(client) or exports.integration:isPlayerScripter(client) then
			...
			local itemID = getElementData(source, "itemID")
			if itemID == 138 and not exports.integration:isPlayerDeveloper(client) then
				outputChatBox("Only a Lead+ admin can move this item.", client, 255, 0, 0)
				reset()
				return
			end
			
			if itemID == 139 and not exports.integration:isPlayerDeveloper(client) then
...
function pickupItem(object, leftammo)
	...
	if not canPickup(source, object) then
		outputChatBox("[!] #ffffffBu öğeyi alamazsınız. F2 ile bir yetkiliye başvurun.", source, 255, 0, 0, true)
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if creator ~= picker then
			...
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("[!] #ffffffVarlıkların aynı hesaptaki karakterleri veya bir oyuncunun sahip olduğu çoklu hesaplar arasında aktarılması kesinlikle yasaktır! ", source, 255, 0, 0, true)
	...
	if itemID == 115 then
		if isThisGunDuplicated(itemValue, source) then
			destroyElement(object)
			dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			outputChatBox("[!] #ffffffSilah çoğaltılması tespit edilerek silahınız silindi, üzgünüz.", source, 255,0,0, true)
			return false
		end
	end
	...
	if itemID == 138 then
		if not exports.integration:isPlayerAdmin(source) then
			outputChatBox("[!] #ffffffBu öğeyi sadece bir yetkili alabilir.", source, 255, 0, 0, true)
			exports.anticheat:changeEld(object, "transfering", nil)
			return false
		end
	end
	...
	if itemID == 134 then -- money
		...
		outputChatBox("" .. exports.global:formatMoney(itemValue) .. "$ alır.", source, 255, 194, 14)
		triggerEvent('sendAme', source, "" .. exports.global:formatMoney(itemValue) .. "$ alır.")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
