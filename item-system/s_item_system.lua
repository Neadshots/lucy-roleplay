...
function removeOOC(text)
	if not text then
		outputChatBox("Error code 0001 occurs, please report to Owlgaming MTA Scripting Team at http://forums.owlgaming.net/forms.php?do=form&fid=1")
		return ""
	end
...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==1) then -- hotdog
			...
			triggerEvent('sendAme', source, "hotdog'u yemeye başlar.")
		...
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				local dbid, entrance, exit, interiorType, interiorElement = exports['interiors']:findProperty( source, found )
		...
		elseif (itemID==10) then -- red dice
			exports.global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " zar atti ((" .. math.random( 1, 6 ) .. "))", 102, 255, 255)
		elseif (itemID==217) then -- red dice
		...
		elseif (itemID==11) then -- taco
			...
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "elindeki Taco'yu yemeye başlar.")
			takeItemFromSlot(source, itemSlot)
			--[[
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
		...
		elseif (itemID==13) then -- donut
			...
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "donut'u yemeye baslar.")
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					setElementData(source, "hunger", 100)
				else
		...
		elseif (itemID==14) then -- cookie
			...
			--[[giveHealth(source, 25)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "kurabiyeleri yemeye baslar.")
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					setElementData(source, "hunger", 100)
				else
		...
		elseif (itemID==15) then -- water
			...
			--[[giveHealth(source, 30)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "su sisesini icmeye baslar.")
			if getElementData(source, "thirst") <= 100 then
				if getElementData(source, "thirst") >= 75 then
					setElementData(source, "thirst", 100)
				else
		...
		elseif (itemID==17) then -- watch
			triggerEvent('sendAme', source, "gözlerini saatine doğru götürür.")
			outputChatBox("[!] #ffffffSaat: " .. string.format("%02d:%02d", getTime()) .. "", source, 0, 55, 255, true)
		...
		elseif (itemID==27) then -- FLASHBANG
			...
			triggerEvent('sendAme', source, "flash bombası fırlatır.")
		...
		elseif (itemID==29) then -- RAM
			--if (getElementData(source, "duty") > 0) and (getElementModel(source) == 285) then
				...
				if not (found) then
					outputChatBox("[!] #ffffffBir kapının yakınında değilsin.", source, 255, 0, 0, true)
				else
					...
					if (interiorStatus[1] ~= 2) and (interiorStatus[4] < 0) and (interiorStatus[3]) and not (interiorStatus[2]) then
						outputChatBox("[!] #ffffffBu kapı bir oyuncuya ait değil.", source, 255, 0, 0, true)
					elseif interiorStatus[2] then
						outputChatBox("[!] #ffffffBu kapı devre dışı.", source, 255, 0, 0, true)
					elseif (interiorStatus[3]) then
						...
						triggerEvent('sendAme', source, "ram kapıya sıkıştırılır ve kapıyı açılır.")
						...
						outputChatBox("[!] #ffffffBu kapı kilitli.", source, 255, 0, 0, true)
					end
				end
			--[[else
		...
		elseif (itemID==35) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "Morfin hapını yutar.")
		...
		elseif (itemID==36) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bir Ecstasy hapı yutar.")
			
			setPedArmor(source, getPedArmor(source) + 15)
			setTimer(
				function(plr)
					if isElement(plr) then
						if (getPedArmor(plr) - 15 < 0) then
							setPedArmor(plr, 0)
						else
							setPedArmor(plr, getPedArmor(plr) - 15)
						end
					end
				end,
			600000, 1, source)
		...
		elseif (itemID==37) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "Heroin injekte eder.")
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.global:sendLocalMeAction(source, "bir esrar tomurcuğu alarak sigaranın içine sarar.")
                            giveItem(source, 181, itemvalue - 1)
                        else
                            outputChatBox( "[!] #ffffffEnvanterin dolu.", source, 255, 0, 0, true)
                        end
                    else
                        outputChatBox("[!] #ffffffSarma paketi boş.", source, 255, 0, 0, true)
                    end
                else
                    outputChatBox("[!] #ffffffBu işlemi yapabilmen için Sarma Paketin olmalıdır.", source, 255, 0, 0, true)
                end
            else
                triggerEvent('sendAme', source, "esrar paketinin içi boş görünüyor.")
            end
			
		elseif (itemID==39) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "Methamphetamine koklar.")
		...
		elseif (itemID==40) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "Epinephrine kalemi injekte eder.")
		...
		elseif (itemID==42) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "kuru mantar yer.")
		...
		elseif (itemID==43) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "PCP hapı yutar.")
		...
		elseif itemID == 48 then -- Briefcase
			outputChatBox("[!]#ffffff Bu sistem artık kullanımda değil!", source, 255, 0, 0, true)
		...
		elseif (itemID==58) then
			...
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "biraz iyi Ziebrand birası içer.")
		...
		elseif (itemID==60) then
			...
			local dimension = getElementDimension(source)
			local retval = call(getResourceFromName("interiors"), "addSafeAtPosition", source, x,y,z, rz) --0 no error, 1 safe already exists, 2 player does not own interior
			if (retval == 0) then
				triggerEvent('sendAme', source, "kasayı yerleştirir.")
				takeItemFromSlot(source, itemSlot)
			elseif (retval == 2) then
				outputChatBox("[!] #ffffffHerhangi bir ev/işyerinde değilsin.", source, 255, 0, 0, true)
			elseif (retval == 3) then
				outputChatBox("[!] #ffffffKasanın yerleştirmeye çalıştığın ev/işyerine sahip olmalısın.", source, 255, 0, 0, true)
			end
		...
		elseif (itemID==83) then -- Coffee
			...
			--[[giveHealth(source, 25)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "kahve'yi icmeye baslar.")
			if getElementData(source, "thirst") <= 100 then
				if getElementData(source, "thirst") >= 75 then
					setElementData(source, "thirst", 100)
				else
		...
		elseif (itemID==92) then
			...
			giveHealth(source, 25)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, " hindi'yi yemeye baslar.")]]
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==93) then
			--exports["wearable-system"]:triggerWearableSystem(source, itemID, itemSlot)
			
			takeItemFromSlot(source, itemSlot)
			--[[giveHealth(source, 22)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
		...
		elseif (itemID==95) then -- Generic Drink
			...
			giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "drinks some " .. itemName .. ".")
			if getElementData(source, "thirst") <= 100 then
				if getElementData(source, "thirst") >= 75 then
					setElementData(source, "thirst", 100)
				else
		...
		elseif (itemID==99) then --[ADDED: 2/22/10 by herbjr] tray
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			triggerEvent('sendAme', source, "tray'i yemeye baslar.")
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					setElementData(source, "hunger", 100)
				else
					setElementData(source, "hunger", getElementData(source, "hunger") + 21)
				end
			end]]
		elseif (itemID==100) then --[ADDED: 2/22/10 by herbjr] milk
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			triggerEvent('sendAme', source, "kucuk sut kutusunu icmeye baslar.")
			takeItemFromSlot(source, itemSlot)
			--[[if getElementData(source, "thirst") <= 100 then
				if getElementData(source, "thirst") >= 75 then
					setElementData(source, "thirst", 100)
				else
					setElementData(source, "thirst", getElementData(source, "thirst") + 20)
				end
			end]]
		elseif (itemID==101) then --[ADDED: 2/22/10 by herbjr] juice
			...
			takeItemFromSlot(source, itemSlot)
			--[[giveHealth(source, 18)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			triggerEvent('sendAme', source, "kucuk meyve suyu kutusunu icmeye baslar.")
			if getElementData(source, "thirst") <= 100 then
				if getElementData(source, "thirst") >= 75 then
					setElementData(source, "thirst", 100)
				else
		...
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Cabbage (Asked for by Misha)
			...
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					...
					setElementData(source, "hunger", getElementData(source, "hunger") + 15)
				end
			end]]
		...
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
                ...
                exports.global:applyAnimation(source, "SMOKING", "M_smk_in", 6000, false, true, true)
		...
		elseif (itemID==108) then -- donut
			...
			takeItemFromSlot(source, itemSlot)
			--[[giveHealth(source, 28)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "pancake'yi yemeye baslar.")
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					setElementData(source, "hunger", 100)
				else
		...
		elseif (itemID==109) or (itemID==110) then -- Fruit
			...
			--[[giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			triggerEvent('sendAme', source, " " .. itemName .. " yemeyi baslar.")
			if getElementData(source, "hunger") <= 100 then
				if getElementData(source, "hunger") >= 75 then
					setElementData(source, "hunger", 100)
				else
		...
		elseif itemID == 286 then -- pizza
			exports.global:applyAnimation(source, "FOOD", "eat_pizza", 3000, true, false, false)
			triggerEvent('sendAme', source, "pizza dilimini yemeye başlar.")
				...
				if getElementData(source, "hunger") <= 100 then
					if getElementData(source, "hunger") >= 75 then
						...
						setElementData(source, "hunger", getElementData(source, "hunger") + 25)
						if getElementData(source, "yaralanma") == 0 then
							giveHealth(source, 20)
						end
					end
				end
		elseif itemID == 287 then -- Şemsiye
			if getElementData(source, "semsiyeDurum") == 0 then
				setElementData(source, "semsiyeDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "semsiye")
				exports.anticheat:changeProtectedElementDataEx(source, "walkingstyle", 57, true)
				setPedWalkingStyle(source, 57)
			elseif getElementData(source, "semsiyeDurum") == 1 then
				setElementData(source, "semsiyeDurum", 0)
				exports.anticheat:changeProtectedElementDataEx(source, "walkingstyle", 118, true)
				setPedWalkingStyle(source, 118)
				
				--triggerEvent("artifacts:remove", source, source, "semsiye")
			end
		elseif itemID == 288 then -- Pembe Şemsiye
			if getElementData(source, "semsiyeDurum") == 0 then
				setElementData(source, "semsiyeDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "semsiyepembe")
				exports.anticheat:changeProtectedElementDataEx(source, "walkingstyle", 57, true)
				setPedWalkingStyle(source, 57)
			elseif getElementData(source, "semsiyeDurum") == 1 then
				setElementData(source, "semsiyeDurum", 0)
				...
				--triggerEvent("artifacts:remove", source, source, "semsiyepembe")
			end
		elseif itemID == 289 then -- Yeşil Şemsiye
			if getElementData(source, "semsiyeDurum") == 0 then
				setElementData(source, "semsiyeDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "semsiyeyesil")
				exports.anticheat:changeProtectedElementDataEx(source, "walkingstyle", 57, true)
				setPedWalkingStyle(source, 57)
			elseif getElementData(source, "semsiyeDurum") == 1 then
				setElementData(source, "semsiyeDurum", 0)
				...
				--triggerEvent("artifacts:remove", source, source, "semsiyeyesil")
			end
		elseif itemID == 290 then -- Kırmızı Şemsiye
			if getElementData(source, "semsiyeDurum") == 0 then
				setElementData(source, "semsiyeDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "semsiyekirmizi")
				exports.anticheat:changeProtectedElementDataEx(source, "walkingstyle", 57, true)
				setPedWalkingStyle(source, 57)
			elseif getElementData(source, "semsiyeDurum") == 1 then
				setElementData(source, "semsiyeDurum", 0)
				...
				--triggerEvent("artifacts:remove", source, source, "semsiyekirmizi")
			end
		elseif itemID == 294 then -- Balkabağı
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "pumpkin")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "pumpkin")
			end
		elseif itemID == 295 then -- At Kafası
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "atkafasiMaske")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "atkafasiMaske")
			end
		elseif itemID == 296 then -- Köpek Kafası
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "kopekkafasiMaske")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "kopekkafasiMaske")
			end
		elseif itemID == 297 then -- Civciv Kafası
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "civcivkafasiMaske")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "civcivkafasiMaske")
			end
		elseif itemID == 298 then -- Kurt Kafası
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "kurtkafasiMaske")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "kurtkafasiMaske")
			end
		elseif itemID == 299 then -- Yılbaşı Şapkası
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
		...
		elseif itemID == 300 then -- Polis Şapkası
			if getElementData(source, "faction") == 1 then 
				...
				elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
					...
					--triggerEvent("artifacts:remove", source, source, "polisSapka")
				end
			else
				outputChatBox("[!] #ffffffLos Santos Polis Departmanı birliğinde olmadığınız için kullanamazsınız.", source, 255, 0, 0, true)
			end
		elseif itemID == 301 then -- Police Cap 1
			if getElementData(source, "faction") == 1 then 
				if getElementData(source, "balkabagiSapkaDurum") == 0 then
					setElementData(source, "balkabagiSapkaDurum", 1)
				...
				elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
					...
					--triggerEvent("artifacts:remove", source, source, "polisCap1")
				end
			else
				outputChatBox("[!] #ffffffLos Santos Polis Departmanı birliğinde olmadığınız için kullanamazsınız.", source, 255, 0, 0, true)
			end
		elseif itemID == 302 then -- Gözlük 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "gozlukTak1")
			end
		elseif itemID == 303 then -- Gözlük 2
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "gozlukTak2")
			end
		elseif itemID == 312 then -- Gözlük 2
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "chicago")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "chicago")
			end
		elseif itemID == 316 then -- Özel Şapka 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "ozelSapka1")
			end
		elseif itemID == 321 then -- Özel Şapka 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "bereSapka1")
			end
		elseif itemID == 322 then -- Özel Şapka 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "bereSapka2")
			end
		elseif itemID == 323 then -- Kış Şapka 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "kisSapka1")
			end
		elseif itemID == 324 then -- Motorcu Kaskı 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "motorcuKask1")
			end
		elseif itemID == 325 then -- Motorcu Kaskı 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "motorcuKask2")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "motorcuKask2")
			end
		elseif itemID == 326 then -- Gözlük yuvarlak cam 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "gozlukCam1")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "gozlukCam1")
			end
		elseif itemID == 327 then -- Gözlük yuvarlak cam 2
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "gozlukCam2")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "gozlukCam2")
			end
		elseif itemID == 328 then -- Gözlük yuvarlak cam 3
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "gozlukCam3")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "gozlukCam3")
			end
		elseif itemID == 329 then -- Gözlük yuvarlak cam 4
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
				
				--triggerEvent("artifacts:toggle", source, source, "gozlukCam4")
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				setElementData(source, "balkabagiSapkaDurum", 0)
				
				--triggerEvent("artifacts:remove", source, source, "gozlukCam4")
			end
		elseif itemID == 330 then -- Siyah Şapka 1
			if getElementData(source, "balkabagiSapkaDurum") == 0 then
				setElementData(source, "balkabagiSapkaDurum", 1)
			...
			elseif getElementData(source, "balkabagiSapkaDurum") == 1 then
				...
				--triggerEvent("artifacts:remove", source, source, "siyahSapka1")
			end
			
		elseif itemID == 313 then -- Ekrana Gösterilecek Resim
			exports["Lucyroleplay-url-link-sistemi"]:loadImageByURL(itemValue, source)
		elseif itemID == 331 then -- Viski
			
			--triggerEvent("artifacts:toggle", source, source, "viskiIC")
		elseif itemID == 332 then -- Sarap
			
			--triggerEvent("artifacts:toggle", source, source, "sarapIC")
		elseif itemID == 333 then -- Yunan Rakisi
			
			--triggerEvent("artifacts:toggle", source, source, "rakiIC")
		elseif itemID == 334 then -- Brooklyn Bira
			
			--triggerEvent("artifacts:toggle", source, source, "biraIC")
		elseif itemID == 335 then -- Rus Vodkasi
			
			--triggerEvent("artifacts:toggle", source, source, "vodkaIC")
		elseif (itemID == 306) then -- Yılbaşı Paketi
		
			takeItemFromSlot(source, itemSlot)
			
		elseif itemID == 56 then
			if getElementData(source, "vip") == 0 then 
				outputChatBox("[!] #ffffffVIP Olmadığınız için bu işlemi yapamazsınız.", source, 255, 0, 0, true)
				return
			end
    	...
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                ...
                exports.global:sendLocalMeAction(source, "bir sarma ruloya bakar.")
		...
		elseif (itemID==214) then
			...
			takeItemFromSlot(source, itemSlot)
		elseif (itemID>=218) and (itemID<=232) then
			--exports["wearable-system"]:triggerWearableSystem(source, itemID, itemSlot)
		...
		elseif (itemID == 285) then -- Paket Pizza
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 286, 1)) then
					...
					outputChatBox( "[!] #ffffffEnvanterinizde boş alan yok.", source, 255, 0, 0 , true)
				end
			else
				triggerEvent('sendAdo', source, "pizza kutusu boş görünüyor.")
			end
		elseif (itemID == 286) then -- Pizza Dilimi
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
			triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " rozetini sol yakasından tutarak çıkartır." )
			setElementData(source, "badgeTakili", 0)
		else
			triggerEvent('sendAme', source, "" .. badges[itemID][1] .. " rozetini sol yakasından tutarak çıkartır.")
			setElementData(source, "badgeTakili", 0)
		end
		...
		for key, badge in pairs ( badges ) do
			if key ~= itemID then
				if ( getElementData ( source, badge[1] ) ) then
					...
					setElementData(source, "badgeTakili", 1)
					else
		...
		triggerEvent('sendAme',  source, "wraps a " .. badges[itemID][1] .. " around their head." )
			...
			setElementData(source, "badgeTakili", 1)
			triggerEvent('sendAme',  source, " " .. badges[itemID][1] .. " isimli badgesini üzerindeki kıyafet/ceket'in sol üst kısmına yapıştırır." )
		end
	end
	exports.global:updateNametagColor(source)
end
...
function christmasCoke(thePlayer)
	...
	if(#santaCars > 0) then
		...
		for k,v in ipairs(santaCars) do
			...
			if math.random(100) >= thisChance then --if no car won
				...
				exports.global:sendLocalMeAction(thePlayer, "finds a car key in "..tostring(gendertext).." christmas coca-cola bottle.")
	...
	if math.random(100) >= 40 then
		...
		exports.global:sendLocalMeAction(thePlayer, "finds a lottery ticket in "..tostring(gendertext).." christmas coca-cola bottle.")
	end
end
...
function christmasLotteryUseTicket(santa)
	...
	if hasItem then
		...
		if prizes[prize] then
			...
			exports.global:sendLocalMeAction(santa, "gives "..tostring(playerName).." a "..tostring(itemName)..".")
		else
