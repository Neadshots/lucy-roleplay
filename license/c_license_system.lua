...
local dominick = createPed(187,  1103.87890625, -767.990, 976.59997558594)
setPedRotation(dominick, 180)
setElementDimension(dominick, 80)
	...
	if isInSlot(x, y, w, 25) then
		...
		if getKeyState("mouse1") and lastClick+200 <= getTickCount() then
			...
			if selectedLicense ~= 0 then
				...
				if exports.global:hasMoney(localPlayer, licensecost) then
					if (license == 1) then
						...
						--	showCursor(false)
						else--if(getElementData(localPlayer,"license.car")==3) then
							removeEventHandler("onClientRender", root, renderLicenseWindow)
							showCursor(false)
							initiateDrivingTest()
						end
					...
					exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Ehliyeti almak için yeterli paranız yok. Fiyat: $"..licensecost)
				end
			else
				exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen listeden ehliyet tipini seçiniz." )
			end
		end
	end
...
function showRecoverLicenseWindow()
	...
	addEventHandler("onClientGUIClick", gui.bRecover, function()
		...
		if (row==-1) or (col==-1) then
			exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen, ehliyet tipini seçiniz!" )
			return false
		end
...
function acceptLicense(button, state)
	if (button=="left") then
		if (source==bAcceptLicense) or (source==bBuyLicense)then
			...
			if (row==-1) or (col==-1) then
				exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Lütfen, listeden ehliyet tipiniz seçiniz!" )
				return false
			end
			...
			if (source==bAcceptLicense) then
				if not exports.global:hasMoney( localPlayer, licensecost ) then
					exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Ehliyeti almak için yeterli paranız yok. Fiyat: $"..licensecost)
					return false
				end
			end
			...
			if source==bAcceptLicense then
				...
				elseif (license == 5) then
					if getElementData(localPlayer, "license.fish") < 0 then
						exports.hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You need to wait another " .. -getElementData(localPlayer, "license.fish") .. " hours before being able to obtain a " .. licensetext .. "." )
		...
		if veh and getVehicleOccupant( veh ) == getLocalPlayer( ) then
			if isElementFrozen( veh ) and getVehicleEngineState( veh ) then
				if getVehicleType(veh) == 'veh' or getVehicleType(veh) == 'Bike' then
					exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Motorun ayaklığını indirmeniz gerekmektedir. /kickstand yazarak indirebilirsiniz." )
				elseif getVehicleType(veh) == 'Boat' then
					exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Çapanızı geri çekmeniz gerekmektedir. /anchor yazarak geri çekebilirsiniz." )
				else
					exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Aracınızın el freni kaldırılmış. /handbrake yazarak indirebilirsiniz." )
				end
			elseif not getVehicleEngineState( veh ) then
				exports.hud:sendBottomNotification(localPlayer, "Motorlu Taşıtlar Departmanı", "Aracınızın motoru çalışmamaktadır. 'J' tuşuna basarak motoru çalıştırabilirsiniz." )
			end
		end
	end
)