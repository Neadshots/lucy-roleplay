...
local SCOREBOARD_Y = math.floor ( ( g_windowSize[2] - SCOREBOARD_HEIGHT ) / 2 ) + 60
-- Scoreboard colors
...
drawBackground = function ()
	...
	dxDrawRectangle ( SCOREBOARD_X, SCOREBOARD_Y,
					  ...
					  tocolor(20, 20, 20, 170), SCOREBOARD_POSTGUI )
	dxDrawImage(SCOREBOARD_X, SCOREBOARD_Y-120, SCOREBOARD_WIDTH, 120, "files/score_top.png")
...
drawScoreboard = function ()
	...
	dxDrawText ( serverName, left, top, right, bottom,
				 SCOREBOARD_SERVER_NAME_COLOR, 1, font, "left", "center",
				 true, false, SCOREBOARD_POSTGUI )


	-- Render the player count
	local usagePercent = (#g_players / maxPlayers) * 100
	local strPlayerCount = "Oyuncu: " .. tostring(#g_players) .. "/100"
	...
	if left < right then
		dxDrawText ( strPlayerCount, left-5, top, right, bottom,
					SCOREBOARD_PLAYERCOUNT_COLOR, 1, font, "left", "center",
					true, false, SCOREBOARD_POSTGUI )
	end
	...
	-- Draw the headers
	drawRowBounded ( "ID", "Karakter Adı", "Seviye",  "Ping",
	...
	local renderEntry = function ( player, seeUsername )
		...
		local playerHours = getElementData( player, 'level' ) or 0
		local playerPing = getPlayerPing ( player )
		playerPing = tostring ( playerPing )
		local r, g, b = 255, 255, 255--getPlayerNametagColor ( player )
		...
		-- Render it!
		drawRowBounded ( playerID, playerName, playerHours, playerPing, colors, font, top )
	end
