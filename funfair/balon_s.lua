...
local biletColShape = createColSphere(395.5283203125, -2065.267578125, 14.758009910583, 5)
local biletx, bilty, biletz = 395.5283203125, -2065.267578125, 14.758009910583
local object = 1212
			...
			if not getElementData(balon, "mesgul") and not getElementData(balon, "isiniyor") then
				if (getElementData(thePlayer, "balonbilet") or 0) > 0 then
					...
					if getElementData(balon, "isiniyor") then
						outputChatBox("[!] #f0f0f0Balon ısınana kadar bekleyin. Bu işlem 7 - 10 saniye sürebilir.", thePlayer, 0, 0, 255, true)
					end
				else
					local x,y,z = getElementPosition(thePlayer)
					outputChatBox("[!] #f0f0f0Balona binmek için biletiniz yok!", thePlayer, 255, 0, 0, true)
	...
	function(thePlayer, cmd)
		--if bilet == "balon" then
			if isElementWithinColShape(thePlayer, biletColShape) then
				--outputChatBox("[!] #f0f0f0Kullanım: /biletal.", thePlayer, 0, 255, 0, true)
				if exports.global:takeMoney(thePlayer, 10) then
					outputChatBox("[!] #f0f0f0Bir adet bilet satın aldınız. Ücret: $10.", thePlayer, 0, 255, 0, true)
					setElementData(thePlayer, "balonbilet", (getElementData(thePlayer, "balonbilet") or 0 ) + 1)
				else
					outputChatBox("[!] #f0f0f0Yeterli paranız bulunmamaktadır. Ücret: $10.", thePlayer, 255, 0, 0, true)
				end
			end
		--end
	end
