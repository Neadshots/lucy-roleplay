		...
		if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
			if showToOffDutyAdmins or (getElementData(thePlayer, "duty_admin") == 1) then
				if getElementData(thePlayer, "report_panel_mod") == "2" or getElementData(thePlayer, "report_panel_mod") == "3" then
					exports["reports"]:showToAdminPanel(message, thePlayer, 255,0,0)
				else
		...
		if isElement( thePlayer ) then
			if #candidates == 0 then
				if not fromBankSystem then
					outputChatBox(exports.pool:getServerSyntax(false, "e").."Kullanıcı bulunamadı.", thePlayer, 255, 0, 0, true)
				end
			else
...
function sendLocalText(rootPlayer, message, r, g, b, distance, exclude, useFocusColors, ignoreDeaths, useHex)
	if not useHex then
		useHex = false
	end
	exclude = exclude or {}
	local affectedPlayers = { }
	local x, y, z = getElementPosition(rootPlayer)
	
	if getElementType(rootPlayer) == "player" and exports['freecam-tv']:isPlayerFreecamEnabled(rootPlayer) then return end
	...
	for index, nearbyPlayer in ipairs(getElementsByType("player")) do
		if isElement(nearbyPlayer) and getDistanceBetweenPoints3D(x, y, z, getElementPosition(nearbyPlayer)) < ( distance or 20 ) then
			...
			if not exclude[nearbyPlayer] and (ignoreDeaths or not isPedDead(nearbyPlayer)) and logged==1 and getElementDimension(rootPlayer) == getElementDimension(nearbyPlayer) then
	...
	if getElementType(rootPlayer) == "player" and #affectedPlayers > 0 then -- TV SHOW!
