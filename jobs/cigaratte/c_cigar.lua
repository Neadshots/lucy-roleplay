...
setElementData(badguy2, "name", "Reginald Watson")
...
function cigarJobDisplayGUI(thePlayer)
	...
	if (ftype) and (ftype == 0) or (ftype == 1) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Reginald Watson fısıldar: Hey, elimde bir iş var. Ne dersin, ha?", 255, 255, 255, 3, {}, true)
		cigarAcceptGUI(thePlayer)
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Reginald Watson diyor ki: Seninle bir işim yok. Derhal toz ol buradan.", 255, 255, 255, 10, {}, true)
		return
	end
end
...
function cigarAcceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Reginald Watson diyor ki: Yandaki vanlardan birini alarak işe başla, araçlar yüklü ve yola çıkmaya hazır. Bol şanslar, ahbap.", 255, 255, 255, 3, {}, true)
			setTimer(function() exports.hud:sendBottomNotification(getLocalPlayer(), "Sigara Kaçakçılığı", "Yandaki beyaz kamyonlardan birini alıp, /sigara basla yazarak işe başlayabilirsiniz!") end, 500, 1)
			return	
		end
	)
...
function cstartJob(cmd, arg)
	if arg == "basla" then
		if not cigarBlip then --not routeMarker then
			...
			exports.hud:sendBottomNotification(localPlayer, "Sigara Kaçakçılığı", "Zaten bir sefere başladınız.")
		end
	end
end
				...
				for _, marker in ipairs(cigarOldMarker) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							exports.hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınıza yeni mallar yükleniyor, lütfen bekleyiniz.")
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									exports.hud:sendBottomNotification(thePlayer, "Sigara Kaçakçılığı", "Aracınıza yeni mallar yüklenmiştir. Gidebilirsiniz.")
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitPlayer, true)
							setElementFrozen(hitVehicle, true)
							exports.hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınızdaki mallar indiriliyor, lütfen bekleyiniz.")
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									exports.hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınızdaki mallar indirilmiştir, geri dönebilirsiniz.")
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									cupdateRoutes()
								end, 1000, 1, hitPlayer, hitVehicle, source
...
function trashAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 459 and vehicleJob == 8 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 8 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için sigara Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
