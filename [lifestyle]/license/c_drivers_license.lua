...
function initiateDrivingTest()
	...
	outputChatBox("#FF9933Dışarı çıkıp test araçlarından birine binin ve testi tamamlayın.", 255, 194, 14, true)
...
function startDrivingTest(element)
	if element == getLocalPlayer() then
		...
		if not (testVehicle[id]) then
			outputChatBox("#FF9933Sınava devam edebilmek için size sağlanan araç ile devam etmeniz gerekiyor.", 255, 0, 0, true ) -- Wrong car type.
		else
			destroyElement(blip)
