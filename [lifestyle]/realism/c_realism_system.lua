...
function adminProtect ( attacker, weapon, bodypart )
	if getElementData(source,"duty_admin") == 1 and exports.integration:isPlayerTrialAdmin(source) then
		cancelEvent()
	end
end
addEventHandler ( "onClientPlayerDamage", getLocalPlayer(), adminProtect )
