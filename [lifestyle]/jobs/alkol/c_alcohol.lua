...
setElementData(badguy, "name", "Igor Bogdanov")
...
function jobDisplayGUI(thePlayer)
	...
	if (ftype) and (ftype < 2) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Igor Bogdanov fısıldar: Hey, elimde bir iş var. Ne dersin, ha?", 255, 255, 255, 3, {}, true)
		acceptGUI(thePlayer)
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Igor Bogdanov diyor ki: Seninle bir işim yok. Derhal toz ol buradan.", 255, 255, 255, 10, {}, true)
		return
	end
end
...
function acceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] " .. getPlayerName(thePlayer):gsub("_", " ") .. " diyor ki: " .. Accept[math.random(#Accept)], 255, 255, 255, 3, {}, true)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] Igor Bogdanov diyor ki: Yandaki kamyonlardan birini alarak işe başla, kamyonlar yüklü ve yola çıkmaya hazır. Bol şanslar, ahbap.", 255, 255, 255, 3, {}, true)
	...
	addEventHandler("onClientGUIClick", cancelBtn, 
		function()
			destroyElement(jobWindow)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] " .. getPlayerName(thePlayer):gsub("_", " ") .. " diyor ki: " .. kabulEtmeMsg[math.random(#kabulEtmeMsg)], 255, 255, 255, 3, {}, true)
			return	
		end
	)
end
...
function startJob(cmd, arg)
	if arg == "basla" then
		if not alcoholBlip then--not routeMarker then
			...
			exports.hud:sendBottomNotification(localPlayer, "Alkol Kaçakçılığı", "Zaten bir sefere başladın.")
		end
	end
end
...
function ickiAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 456 and vehicleJob == 9 then
		if thePlayer == getLocalPlayer() and seat ~= 1 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 9 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için alkol Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
