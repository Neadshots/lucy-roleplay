local copguy = createPed(16, 1673.3251953125, -1882.83984375, 13.546875,85, true)
setElementData(copguy, "talk", 1)
setElementData(copguy, "name", "John River")
...
function copJobDisplayGUI()
	...
	if (carlicense~=1) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] John River diyor ki: Bu mesleği icra edebilmek için ehliyet sahibi olmalısınız.", 255, 255, 255, 3, {}, true)
		return
	end

	if (crimecount == 0) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] John River diyor ki: Müracaatınızda işi almanıza bir engel bulunamamıştır.", 255, 255, 255, 3, {}, true)
		copAcceptGUI(getLocalPlayer())
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] John River diyor ki: Maalesef ki siciliniz bu işi yapmaya müsait değil.", 255, 255, 255, 10, {}, true)
		return
	end
end
...
function copAcceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "[Ingilizce] John River diyor ki: Yandaki çöp arabalarından birini alarak işe başla, dikkatli olmayı unutma.", 255, 255, 255, 3, {}, true)
			setTimer(function() exports.hud:sendBottomNotification(getLocalPlayer(), "Çöp Taşımacılığı", "Yandaki çöp arabalarından birini alıp, /copculukbasla yazarak işe başlayabilirsiniz!") end, 500, 1)
			return	
		end
	)
...
function trashBasla(cmd)
	if not getElementData(getLocalPlayer(), "trashSoforlugu") then
		...
		exports.hud:sendBottomNotification(localPlayer, "Çöp Şöförlüğü", "Çöp seferine başlamak için meslek aracında bulunmalısınız. (Çöpçülük)")
		end
	else
				...
				for _, marker in ipairs(trashCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!] #FFFFFFAracınıza yeni mallar yükleniyor, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /trashbitir yazınız.", 0, 0, 255, true)
							setTimer(
