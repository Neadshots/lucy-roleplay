				...
				if toplamyem > 0 then
					...
					setTimer(function(thePlayer) 
						...
						if rastgeleSayi == 2 then
							...
							exports["item-system"]:giveItem(thePlayer, 273, givenFishTable.name)
...
addCommandHandler("balikdurum", 
	function(thePlayer, cmd)
		...
		local toplamBalik = exports["item-system"]:countItems(thePlayer, 273)
...
function balikSat(thePlayer, cmd)
	local countOfPlayerFish = exports["item-system"]:countItems(thePlayer, 273)
	local countPrice = 0
	local playerFishs = exports["item-system"]:getItems(thePlayer)
	...
	if isElementWithinColShape(thePlayer, yemCol) then
		...
		if toplambalik <= 0 then
			...
			exports.global:giveMoney(thePlayer, countPrice)
			for i = 0, countOfPlayerFish do
				exports["item-system"]:takeItem(thePlayer, 273)
			end
