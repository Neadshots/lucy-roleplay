...
function takeOutATMCard(theATM, item)
	if exports["item-system"]:takeItem(theATM, item[1], item[2]) then
		if exports["item-system"]:giveItem(source, item[1], item[2]) then
			triggerEvent('sendAme',  source, "ATM makinesinin yuvasından bir ATM kartı çıkardı." )
			playAtmInsert(theATM) --SoundFX
		else
			triggerEvent('sendAme',  source, "ATM kartını ATM makinesinin yuvasından dışarı kaydırmaya çalıştı." )
...
function applyForNewATMCard(replacingOldATMCard, limitType)
			...
			function(qh, source)
				...
				if rows > 0 then
					...
					if check["card_id"] then
						exports.hud:sendBottomNotification(source, "Maxime Du Trieux says:", "'Üzgünüm,"..playerGender.." ama birden fazla ATM kartına izin vermiyoruz'")
						triggerClientEvent(source, "atm:atmCardExisted", source)
						return false
					end
				end
			end,
		{source}, mysql:getConnection(), "SELECT * FROM `atm_cards` WHERE `card_type`='1' AND `card_owner`='"..charID.."' ")
	else
	...
	elseif limitType == 3 then
		if not exports.global:takeMoney(source, 1500) then
			exports.hud:sendBottomNotification(source, "Maxime Du Trieux says:", "'Could I have $1500 please, "..playerGender.."'.")
			return false
		end
	else
	...
	local makeNewCard = dbExec(mysql:getConnection(),"INSERT INTO `atm_cards`(`card_owner`, `card_number`, `limit_type`) VALUES ('"..tostring(charID).."', '"..numSum.."', '"..limitType.."') ")
	...
	if limitType == 1 then
		triggerEvent('sendAme', source, "bazı dolar notları alır ve bunları Maxime Du Trieux'e verir..")
	end
	exports.global:sendLocalText(source, "* Maxime Du Trieux verir "..tostring(playerName):gsub("_"," ").." şık bir plastik ATM kartı.", 255, 51, 102, 30, {}, true)

	exports.hud:sendBottomNotification(source, "Maxime Du Trieux says:", "'Teşekürler,"..playerGender.."! İşte ATM kartınız (Kart numarası: '"..numSum.."', PIN: '0000').'")
...
function lockATMCard()
	...
	dbQuery(
		function(qh, source)
			...
			local lockATMCard = dbExec(mysql:getConnection(),"UPDATE `atm_cards` SET `card_locked`='1' WHERE `card_id` = '".. cardID .."' ")
	...
	{source}, mysql:getConnection(), "SELECT `card_id`, `card_locked`, `card_pin`, `card_number` FROM `atm_cards` WHERE `card_type`='1' AND `card_owner`='"..tostring(charID).."' ")
end
...
function unlockATMCard()
	...
	dbQuery(
		function(qh, source)
			...
			local lockATMCard = dbExec(mysql:getConnection(),"UPDATE `atm_cards` SET `card_locked`='0' WHERE `card_id` = '".. cardID .."' ")
			if not lockATMCard then
				exports.hud:sendBottomNotification(source, "SQL Error", "An SQL Error CODE03 has occured in Bank System. Please report this to admin Maxime.")
				return false
			end
	...
	{source}, mysql:getConnection(), "SELECT `card_id`, `card_locked`, `card_pin`, `card_number` FROM `atm_cards` WHERE `card_owner`='"..tostring(charID).."' ")
end
	...
	dbQuery(
		function(qh, source)
			...
			if rows > 0 then
				...
				{source}, mysql:getConnection(), "SELECT `bankmoney` from `characters` WHERE `id`= (SELECT `card_owner` FROM `atm_cards` WHERE `card_number`='".. cardNumber .."') ")
				
			end
		end,
	{source}, mysql:getConnection(), "SELECT `card_locked`, `card_pin`, `card_number` FROM `atm_cards` WHERE `card_number`='"..cardNumber.."' AND `card_pin`='"..enteredCode.."' ")
...
function changePIN(theATM, enteredCode)
	...
	if not dbExec(mysql:getConnection(),"UPDATE `atm_cards` SET `card_pin`='"..enteredCode.."' WHERE `card_number`='"..cardNumber.."' ") then
...
function withdrawATMMoneyPersonal(amount, theATM)
	...
	if not isThisTransactionWithinLimitation then
		exports.hud:sendBottomNotification(client, "ATM Machine", reason)
		return false
	end
	...
	dbQuery(
		function(qh, client)
			...
			local money = balance - amount
			if money >= 0 then
				if updateBankMoney(client, charID, amount, "minus") then--Update the card Owner's bankmoney (ElementData and SQL)
					exports.global:giveMoney(client, amount) -- Fixed an error in your script, you put the giveMoney twice and you could dupe cash. - Adams
				else
					outputDebugString("SQL error code x2367jk")
					return false
				end
				...
				exports.logs:dbLog(client, 25, client, "WITHDREW $" .. amount.." FROM ("..cardOwner..", "..cardNumber..", "..atmName..")")
			else
	...
	{client}, mysql:getConnection(), "SELECT `bankmoney`, `id` FROM `characters` LEFT JOIN `atm_cards` ON `characters`.`id`=`atm_cards`.`card_owner` WHERE `card_number`='"..cardNumber.."' AND `card_locked`='0' AND `card_type`='1' ")
end
...
function depositATMMoneyPersonal(amount, theATM)
	...
	dbQuery(
		function(qh, client)
			...
			if rows > 0 then
				...
				local charID = balanceCheck["id"]
				if not updateBankMoney(client, charID, amount, "plus") then --Update the card Owner's bankmoney (ElementData and SQL) -- Fixed this too, you set it to minus, so when you put money IN it would come OUT - Adams
					exports.global:giveMoney(client, amount)
					outputDebugString("SQL error code bnskdnf54")
					return false
				end
	...
	mysql:getConnection(), "SELECT `bankmoney`, `id` FROM `characters` LEFT JOIN `atm_cards` ON `characters`.`id`=`atm_cards`.`card_owner` WHERE `card_number`='"..cardNumber.."' AND `card_locked`='0' AND `card_type`='1' ")
end
...
function transferATMMoneyToPersonal(theATM, amount,targetBankAccNo, reason) 
	...
	if not isThisTransactionWithinLimitation then
		exports.hud:sendBottomNotification(client, "ATM Machine", reason2)
		return false
	end
	...
	if faction then
		...
		dbQuery(
			function(qh, client)
				...
				if rows > 0 then
					...
					local sender_net = sender_bankmoney - amount
					if sender_net < 0 then
					...
					if updateBankMoney(client, sender_id, amount, "minus")  then
						if not updateBankMoney(client, target_id, amount, "plus") then 
							outputDebugString("SQL error code GvddGh42")
							triggerEvent("bank:ejectATMCard", client, theATM)
							return false
						end
					else
		...
		{client}, mysql:getConnection(), "SELECT `bankmoney`, id, charactername FROM `characters` LEFT JOIN `atm_cards` ON `characters`.`id`=`atm_cards`.`card_owner` WHERE `card_number`='"..cardNumber.."' AND `card_locked`='0' AND card_type='1' ")
	else 
		...
		dbQuery(
			function(qh, client)
				...
				if rows > 0 then
					...
					dbQuery(
						function(qh, client)
							...
							if rows > 0 then
								...
								if updateBankMoney(client, sender_id, amount, "minus")  then
									if not updateBankMoney(client, target_id, amount, "plus") then
										outputDebugString("SQL error code GvddGhsdsd4")
										triggerEvent("bank:ejectATMCard", client, theATM)
										return false
									end
								else
					...
					{client}, mysql:getConnection(), "SELECT id, `charactername`, bankmoney FROM `characters` LEFT JOIN `atm_cards` ON `characters`.`id`=`atm_cards`.`card_owner` WHERE `atm_cards`.`card_number`='"..toSQL(targetBankAccNo).."' AND `card_locked`='0' AND card_type='1' ")
				end
			end,
		{client}, mysql:getConnection(), "SELECT `bankmoney`, id, charactername FROM `characters` LEFT JOIN `atm_cards` ON `characters`.`id`=`atm_cards`.`card_owner` WHERE `card_number`='"..cardNumber.."' AND `card_locked`='0' AND card_type='1' ")
	end
...
function tellATMTransfers(source, cardInfo, event)
	...
	{source}, mysql:getConnection(), "SELECT w.*, a.charactername as characterfrom, b.charactername as characterto,w.`time` - INTERVAL 1 hour as 'newtime' FROM ATM_wiretransfers w LEFT JOIN characters a ON a.id = `from` LEFT JOIN characters b ON b.id = `to` WHERE " .. (where) .. " ORDER BY id DESC LIMIT 40")
end
