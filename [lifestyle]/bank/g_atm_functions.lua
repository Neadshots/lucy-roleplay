...
function disableATMCard(carNo)
	dbExec(mysql:getConnection(), "UPDATE `atm_cards` SET `card_locked`='1' WHERE `card_number`='"..carNo.."' ")
end
...
function isThisTransactionWithinLimitation(cardNumber,amount, limitType)
	...
	if not withdraws[cardNumber] then
		withdraws[cardNumber] = 0
		setTimer(function()
			withdraws[cardNumber] = 0
		end, limitedInterval, 1)
	end

	if withdraws[cardNumber]+amount > limits[limitType] then
		return false, "You can only withdraw/transfer $"..exports.global:formatMoney(limits[limitType]).." from this ATM card a day ((Every "..math.floor(limitedInterval/60/60/1000).." hours))\n Thus far you have $"..exports.global:formatMoney(limits[limitType]-withdraws[cardNumber]).." available at the moment."
	else
		withdraws[cardNumber] = withdraws[cardNumber] + amount
		return true
	end
