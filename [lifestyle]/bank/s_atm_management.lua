	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		elseif (tonumber(withdraw)) and (tonumber(deposit)) then
			...
			if (tonumber(deposit) > 1) or (tonumber(deposit) < 0) then
				...
				local id = dbExec(mysql:getConnection(),"INSERT INTO atms SET x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', dimension='" .. (dimension) .. "', interior='" .. (interior) .. "', rotation='" .. (rotation) .. "', deposit='" ..(depositlimit).. "', `limit`="..(withdrawlimit))
...
function createFastATM(thePlayer, commandName)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		if isElementWithinColShape(thePlayer, noSpawnZone) then
			...
			local id = dbExec(mysql:getConnection(),"INSERT INTO atms SET x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', dimension='" .. (dimension) .. "', interior='" .. (interior) .. "', rotation='" .. (rotation) .. "',`limit`=5000")
