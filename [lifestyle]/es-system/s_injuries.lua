...
function knockout()
	if playerInjuries[source] and not isTimer( playerInjuries[source]['knockout'] ) then
		...
		exports.global:sendLocalMeAction(source, "has been knocked out.")
	end
end
...
function injuries(attacker, weapon, bodypart, loss)
	...
	--[[if ( bodypart == 3 and loss >= 15 ) or bodypart == 7 or bodypart == 8 then -- damaged either left or right leg
		...
		exports.global:sendLocalMeAction(source, "'s " .. ( bodypart == 7 and "left" or "right" ) .. " leg was hit.", source)
	elseif bodypart == 5 or bodypart == 6 then -- damaged either arm
		...
		exports.global:sendLocalMeAction(source, "'s " .. ( bodypart == 5 and "left" or "right" ) .. " arm was hit.", source)
	elseif bodypart == 3 then
		if not weapon == 17 then
		exports.global:sendLocalMeAction(source, "'s torso was hit.", source)
	elseif bodypart == 9 then -- headshot
		if not attacker or weapon ~= 24 or getElementData(attacker, "deaglemode") ~= 0 then
			if weapon == 25 or weapon == 26 or weapon == 27 or weapon == 30 or weapon == 31 or weapon == 33 or weapon == 34 or weapon == 35 or weapon == 36 or weapon == 38 then
				setPedHeadless(source, true)
				exports.global:sendLocalMeAction(source, "'s head was blown off by " .. getPlayerName(attacker):gsub("_", " ") .. ".", source)
				killPed(source, attacker, weapon, bodypart)
			return
			else
			-- Removed, needs a trigger so done below in doHeadHit and in c_injuries.lua
			return
			end
		end
	end]] -- Disabled limb injuries, Adams
...
function doHeadHit(headHitPlayer)
	...
	exports.global:sendLocalMeAction(headHitPlayer, "'s head was hit.")]]
end
...
addCommandHandler("examine", lvesExamine, false, false)]] -- Disabled cause injuries removed