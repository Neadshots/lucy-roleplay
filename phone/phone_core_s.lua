...
function applyPhone(string, popOutOnPhoneCall)
    ...
    items = exports["item-system"]:getItems(source)
    phoneName = "iPhone 7s"
    for index, row in ipairs(items) do
        if row[1] == 2 then
            phoneName = exports["item-system"]:getItemName(2, row[2], row[5])
        end
    end
            ...
            if callerphoneIsTurnedOn == 0 then
                ...
                elseif getElementData(thePlayer, "injuriedanimation") then
                    ...
                    -- Otherwise find a fool to answer it
                        ...
                        if bankMoney >= 1 then
                            if not exports.anticheat:changeProtectedElementDataEx(thePlayer, "bankmoney", tonumber(bankMoney) - 1, false) then
