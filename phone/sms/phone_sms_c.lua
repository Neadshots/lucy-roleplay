...
function receiveOneSMSThreadFromServer(fetchForPhone, messageSentTo, SMSs, outGoing)
	...
	elseif smsViewingMode[fetchForPhone] == "All" then
		...
		if unreads > 0 then
			addToUnreadSMSs(fetchForPhone, unreads)
		end
	end
end
...
function drawAllSMSThreads(xoffset, yoffset)
	...
	if sortedThreads[phone] and smsThreadInitiated[phone] then
		...
		if #sortedThreads[phone] > 0 then
			...
			for i = 1, #sortedThreads[phone] do
				...
				if thread then
					...
					guiCreateLabel(0.5, 0.5, 1, 0.5, "Hiç mesaj almamışsın.", true, wSMS)
					isEmpty = true
					break
				end
			end
		...
		guiCreateLabel(0.5, 0.5, 1, 0.5, "Mesajlarınız yükleniyor..", true, wSMS)
	end
end
...
function drawSMSComposer(xoffset, yoffset)
	if isPhoneGUICreated() then
		...
		limit = 120
		wSMSComposer = guiCreateScrollPane(30+xoffset, 100+yoffset, 190, 65, false, wPhoneMenu)
		...
		guiSetProperty(smsComposerGUIs.send,"AlwaysOnTop","True")
...
function addToUnreadSMSs(fromPhone, newValue)
	totalUnreadSMSs[fromPhone] = (totalUnreadSMSs[fromPhone]) or 0 + newValue
	if not smsViewingMode[fromPhone] then
