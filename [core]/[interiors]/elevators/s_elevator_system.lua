...
function isInteriorLocked(dimension)
	local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT type, locked FROM `interiors` WHERE id = " .. (dimension)), -1)
	local locked = false
	if result then
...
function enterElevator(goingin)
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		local dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "interiors" ), "findProperty", player, otherCP[INTERIOR_DIM] )
		if dbid > 0 then

		else
			dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "interiors" ), "findProperty", player, currentCP[INTERIOR_DIM] )
		end
		...
		if vehicle then
			...
			if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
				...
				exports.logs:dbLog("SYSTEM", 31, { interiorElement, player, vehicle } , "ENTERED/EXITED")
		...
		elseif isElement(player) then
			if movingInSameInt then
				...
				exports["interiors"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
			end
			return true
		else
	...
	if isElement(vehicle) then
		...
		for i = 0, getVehicleMaxPassengers( vehicle ) do
			...
			if player then
				...
				--exports["interiors"]:setPlayerInsideInterior(interiorElement, player, other )
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
		if (not tonumber(intID) or tonumber(intID)%1~= 0 ) and commandName ~= "PROPERTYCLEANUP" then
			...
			if tonumber(intID) == 0 and not exports.integration:isPlayerSeniorAdmin(thePlayer) then
...
function SmallestElevatorID( ) -- finds the smallest ID in the SQL instead of auto increment
	local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT MIN(e1.id+1) AS nextID FROM elevators AS e1 LEFT JOIN elevators AS e2 ON e1.id +1 = e2.id WHERE e2.id IS NULL"),-1)
	if result then
