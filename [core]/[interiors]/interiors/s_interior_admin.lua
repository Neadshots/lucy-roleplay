		...
		if entrance then
			...
			if theFee >= 0 then
				if interiorStatus[INTERIOR_TYPE] == 1 then
					if exports.integration:isPlayerSeniorAdmin( thePlayer ) or interiorStatus[INTERIOR_OWNER] == getElementData( thePlayer, "dbid" ) then
						...
						if canHazFee then
							...
							if query then
								...
								exports.anticheat:changeProtectedElementDataEx(interiorElement, "entrance", interiorEntrance, true)
				...
				if dbid ~= 0 then
					...
					for key, value in pairs( exports.pool:getPoolElementsByType('player') ) do
						if isElement( value ) and getElementDimension( value ) == dbid then
							...
							--exports.anticheat:changeProtectedElementDataEx( value, "interiormarker" )
							exports.anticheat:changeProtectedElementDataEx(value, "interiormarker", false, false, false)
...
addCommandHandler("moveinterior",
	function(player, cmd, interiorID)
		if (exports.integration:isPlayerHeadAdmin(player) or exports.integration:isPlayerScripter(player)) then
			local interiorID = tonumber(interiorID)
			x, y, z = getElementPosition(player)
			dbExec(exports.mysql:getConnection(), "UPDATE interiors SET x = ?, y = ?, z = ? WHERE id = ?", x, y, z, interiorID)
			outputChatBox(exports.pool:getServerSyntax(player, false,"s").."Mülkün giriş pozisyonu başarıyla ayarlandı.", player, 255, 255, 255, true)
...
function removeInterior(thePlayer, commandName, houseID)
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) or commandName == "MOVETOLS" then
		if not showLoadingProgressTimer then	
...
function cancelRemoveDeletedInts(thePlayer)
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) and getElementData(thePlayer, "confirm:removeDeletedInteriors") then	
...
function cancelForceSellAllInactiveInteriors(thePlayer)
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) and getElementData(thePlayer, "confirm:forceSellAllInactiveInteriors") then	
...
function cancelRemoveForSaleInts(thePlayer)
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) and getElementData(thePlayer, "confirm:removeForSaleInteriors") then	
...
function cancelRemoveInactiveInteriors(thePlayer)
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) and getElementData(thePlayer, "confirm:removeInactiveInteriors") then	
...
function changeInteriorName( thePlayer, commandName, ...)
	if (exports.integration:isPlayerSeniorAdmin(thePlayer)) then -- Is the player an admin?
