...
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		...
		if (ownerName=="None") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Kiralamak için E tuşuna basın "..exports.pool:getServerMoneyType(localPlayer) .. tostring(exports.global:formatMoney(cost)) .. ".", true)
		...
		elseif (ownerName=="None") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Satın almak için E tuşuna basın "..exports.pool:getServerMoneyType(localPlayer) .. tostring(exports.global:formatMoney(cost)) .. ".", true)
			guiSetFont(gBuyMessage, "default")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		else
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	if isHouse then
		...
		taxtax = cost * theTax
		guiSetText(purchaseProperty.label[10], ""..exports.pool:getServerMoneyType(localPlayer) ..exports.global:formatMoney(taxtax).."")
	...
	guiSetText(purchaseProperty.label[8], ""..exports.pool:getServerMoneyType(localPlayer) ..exports.global:formatMoney(cost).."")
end
