...
function options_showmenu()
	...
	if getResourceFromName("donators") then
		bStore = guiCreateButton(margin, wHeight, 230, 30, "Premium Özellikleri", false, wOptions)
		--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
	...
	bLogout = guiCreateButton(margin, wHeight, 230, 30, "Sunucudan Ayrıl", false, wOptions)
	...
	bClose = guiCreateButton(margin, wHeight, 230, 30, "Arayüzü Kapat", false, wOptions)
	addEventHandler("onClientGUIClick", bClose, options_closemenu, false)
	wHeight = wHeight + bHeight

	guiSetSize(wOptions, windowWidth, wHeight, false)
