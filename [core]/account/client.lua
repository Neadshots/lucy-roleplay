	...
	function (statusCode, additionalData, pChars)
		if (statusCode == 0) then
			...
			if #characterList == 0 then
				--LoginScreen_showWarningMessage( "Oops! Artık karakterleri forumdan açabilirsiniz.\nYeni karakter oluşturmak için www.lucyrpg.com ziyaret edin." )
...
function applyClientConfigSettings()
	...
	localPlayer:setData("cFontPro", cFontPro or "default", false)
...
function Exclusive.loginStart()
	...
	bgMusic = playSound("http://185.247.137.160/music.mp3", true)
...
function drawnLogin()
	...
	dxDrawImage(sx/2-256/2, sy/2-256/2-100, 256, 256, "img/logo_2.png", 0, 0, 0, tocolor(255, 255, 255, math.min(255, animation.alpha[animation.step])))
	...
	--[[
	w, h = 376, 81
	dxDrawImage(10, sy-100, w, h, "img/arya.png")
	if isInBox(10, sy-100, w, h) then
		if getKeyState("mouse1") and lastClick+200 <= getTickCount() then
			lastClick = getTickCount()
			triggerServerEvent("redirect:arya",localPlayer,localPlayer)
		end
	end
	]]--
...
local function camRender()
	...
	setCameraMatrix(x1+30+cameraMatrix2, y1+20+cameraMatrix, z1+2, vehicle:getRotation():getX(), vehicle:getRotation():getY(), vehicle:getRotation():getZ(), 20+cameraMatrix, -55)
end
...
function Exclusive.drawnCharacters(table)
	...
	destroyTimer = setTimer(
		function()
			...
			showCursor(true);
			lastArrayClick = 0
			font2 = DxFont(":interface/fonts/FontAwesome.ttf")
...
function drawnCharacters()
	...
	for index, value in ipairs(accountCharacters) do
		...
		dxDrawRectangle(x+border, y+border, w-border*2, h-border*2, tocolor(r, g, b, a));

		if fileExists("img/"..value[9]..".png") then
			dxDrawImage(x+border, y+border, 30-border*2, 30-border*2, "img/"..value[9]..".png");
		end
		dxDrawText(value[2]:gsub("_", " ").." olarak oyna!", x, y, w+x, y+h, tocolor(255, 255, 255), 1, font2, 'center', 'center')
	...
	dxDrawText("Karakter Oluştur", x, y, w+x, y+h, tocolor(255, 255, 255), 1, font2, 'center', 'center')

	dxDrawImage(sx/2-256/2, sy/2-256/2-100, 256, 256, "img/logo_2.png", 0, 0, 0, tocolor(255, 255, 255))
end
function characters_onSpawn(fixedName, adminLevel, gmLevel, location)
	...
	setTimer(
		...
		outputChatBox("(( Sunucuya hoş geldin, "..fixedName.." ))", 245, 246, 250, true)
		outputChatBox("(( Sunucu üzerinde değişiklikler yaptık, öğrenmek için /yenisürümliste ))", 245, 246, 250, true)
...
local updateList = {"Rapor atmak veya soru sormak için /rapor - /sorusor", "Artık kendi adınızı göremeyeceksiniz", "Interior sistemi güncellendi"}
addCommandHandler("yenisürümliste",
	function(cmd)
		for i, v in ipairs(updateList) do
			outputChatBox("(( "..v.." ))", 245, 246, 250, true)
		end
	end
)
...
function creationSteps.startSteps(currStep)
	if currStep == 1 then
		...
		--in the ped,
		--1427.2587890625, 1448.7236328125, 10.8816614151
...
function Exclusive.createGUI()
	...
	addEventHandler("onClientGUIClick", gui['langWindowCloseBtn'], function(button)
		if (source == gui['langWindowCloseBtn'] and button == "left") then
			...
			triggerServerEvent("accounts:characters:new", localPlayer, pname, 'LUCY RPG', race, gender, skin, pheight, pweight, page, languageselected, 1, 1, { 1922.9072265625, -1760.6982421875, 13.546875, 0, 0, 0, ""})
