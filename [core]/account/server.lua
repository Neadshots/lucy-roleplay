...
function resourceStart(resource)
	...
	setRuleValue("Açıklama", "Lucy Roleplay")
...
function loginCallback(queryHandler,source,username,password)
	...
	if rows > 0 then
		...
		setElementDataEx(source, "bakiyeMiktar", tonumber(row["bakiyeMiktari"]) or 0, true)
		...
		function(qh, client, characterID, remoteAccountID, theAdmin, targetAccountName, location)
			...
			if rows > 0 then
				...
				if characterData then
					...
					setElementDataEx(client, "vip", tonumber(characterData["vip"]))
					setElementDataEx(client, "vip_day", tonumber(characterData["vip_day"]))
					setElementDataEx(client, "vip_hour", tonumber(characterData["vip_hour"]))
...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			local bakiyeMiktar = getElementData(player, "bakiyeMiktar") or 0
		
			local update = dbQuery(mysql:getConnection(), "UPDATE characters SET online='0', hunger='" .. (hunger) .. "', thirst='" .. (thirst) .. "', x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', rotation='" .. (rot) .. "', health='" .. (health) .. "', armor='" .. (armor) .. "', dimension_id='" .. (dimension) .. "', interior_id='" .. (interior) .. "', " .. (money) .. (businessprofit) .. "lastlogin=NOW(), lastarea='" .. (zone) .. "', timeinserver='" .. (timeinserver) .. "', alcohollevel='".. ( tostring( alcohollevel ) ) .."' WHERE id=" .. (getElementData(player, "dbid"))) -- , d_addiction='" .. (d_addiction) .. "'
			if not (update) then
			...
			local update2 = dbQuery(mysql:getConnection(), "UPDATE accounts SET bakiyeMiktari='"..bakiyeMiktar.."', lastlogin=NOW() WHERE id = " .. (getElementData(player,"account:id")))
			if not (update2) then
	...
	local characters = {}
		dbQuery(
			function(qh, source)
				...
				setElementData(source, "account:characters", characters)
				triggerClientEvent(source, "accounts:login:attempt", source, 0 )
				triggerEvent( "social:account", source, tonumber( getElementData(client, "account:id") ) )
			end,
