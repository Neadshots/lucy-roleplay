...
function startGB(blaster)
    ...
    local splitValue = split(tostring(getElementData(blaster, "itemValue")), ';')
    local station = tonumber(splitValue[1]) or tostring(splitValue[1]) or 1
    local volume = (tonumber(splitValue[2]) or 100) / 100
	...
	elseif type(station) == "string" then
		...
		if isYoutubeURL(station) then
			...
			if volume > 0 then
				...
				local px, py, pz = getElementPosition(getLocalPlayer())
				if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<50) then
					--local sound = playSound3D("https://mp3donusturucu.com/indir/"..station, x, y, z)
					--local url = station
                    outputChatBox(" Audio Streaming: "..station, 200, 200, 200)
					local sound = playSound3D("http://www.convertmp3.io/fetch/?video="..station, x, y, z)
					if getElementModel(source) == 2232 then -- larger speaker
						setSoundMaxDistance(sound, 400)
					else
						setSoundMaxDistance(sound, 60)
...
addEventHandler ( "onClientPreRender", root,
    function ( )
        for i,v in pairs ( blasters ) do
            if not isElement(v.sound) or getElementDimension ( localPlayer ) ~= getElementDimension ( v.sound ) then break end
            ...
            if sx and distance <= 10 then
				...
				local song = "Bilgi Mevcut Değil"
                if isElement ( v.sound ) then
					if not isYoutubeURL(itemValue) then
						song = getSoundMetaTags ( v.sound )["stream_title"]
					else
						song = getSoundMetaTags ( v.sound )["title"]
					end
                end
                local text = "YouTube Oynatıcı"
				if not isYoutubeURL(itemValue) then
					local streams = exports.carradio:getStreams()
					text = "#" .. itemValue .. " - " .. streams[itemValue][1]
				end
                dxDrawText ( text, sx, sy, sx, sy, tocolor ( 255, 255, 255, 255 ), 0.85, "default-bold", "center" )
                
                if type ( song ) == "string" then
                    dxDrawText ( "Şuanda Çalıyor: " .. song, sx, sy + 15, sx, sy, tocolor ( 255, 255, 255, 255 ), 0.85, "default-bold", "center" )
                end
            end
        end
    end )
