...
addEvent("marijuanaEffect5", true)
addEventHandler("marijuanaEffect5", getRootElement(), doDrug5Effect)
...
local drugMessages = { "Bana doğru sessizce yaklaş", "Waow adamım, sen uçuyorsun!", "Dans etmek ister misin, orospu çocuğu?", "Polisleri arayayım.", "Kaldır kıçını zenci", "Nereye sıçacaklar!", "At gibi kokuyorsun!", "Hadi şimdi beni becer", "Tadıma bakmak istemez misin?" }
	...
	for key, value in ipairs(players) do
		if (value~=localPlayer) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<15) then
				...
				outputChatBox(charname .. " diyor ki: " .. drugMessages[rand2], 255, 255, 255)
				break
			end
		end
	end
...
function uyusturucuKrizi()
	local bagimlilikOrani = getElementData(getLocalPlayer(), "uyusturucu_bagimlilik") or 0
	local krizYok = not getElementData(getLocalPlayer(), "uyusturucu_kriz")
	if krizYok then
	--	return
	end
	...
	elseif bagimlilikOrani <= 50 and bagimlilikOrani > 0 then
		...
		setTimer(setCameraShakeLevel, 5000, 1, 0)
	...
	elseif bagimlilikOrani <= 80 and bagimlilikOrani > 0 then
		setElementHealth(getLocalPlayer(), getElementHealth(getLocalPlayer()) - 10)
		setTimer(fadeCamera, 1000, 1, true)
		setTimer(fadeCamera, 2000, 1, false)
		setTimer(fadeCamera, 3000, 1, true)
		setTimer(fadeCamera, 4000, 1, false)
		...
		setTimer(fadeCamera, 12000, 1, true)
		setCameraShakeLevel(250)
		setTimer(setCameraShakeLevel, 10000, 1, 0)
		outputChatBox("** #f0f0f0Canın uyuşturucu almak istiyor. Gözlerin kapanıyor ve dengeni kaybediyorsun.", 255, 0, 0, true)
...
--setTimer(uyusturucuKrizi, 40000,0)
setTimer(uyusturucuKrizi, 1200000, 0)

addCommandHandler("krizuyusturucu", uyusturucuKrizi)