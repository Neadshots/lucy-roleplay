...
function showItemList()
	if exports.integration:isPlayerDeveloper(localPlayer) then
	if not (wItemsList) then
			local width, height = 800,600
			local scrWidth, scrHeight = guiGetScreenSize()
			local x = scrWidth/2 - (width/2)
			local y = scrHeight/2 - (height/2)
			wItemsList = guiCreateWindow(x, y, width, height,"Lucy Roleplay | Eşya oluşturma Sistemi",false)
			...
			lSpawnTo = guiCreateLabel(11,22,138,19,"Oluşturulacak Kişi: ",false,wItemsList)
			...
			bSkinID = guiCreateButton(458,22,111,23,"Skin ID listesi",false,wItemsList)
			bVisualMode = guiCreateButton(458,45,111,23,"Visualized Modu",false,wItemsList)
			lItemSearch = guiCreateLabel(308,22,140,19,"Eşya Ara:",false,wItemsList)
			bItemSearch = guiCreateEdit(309,41,140,21,"Esya Ismi veya Aciklama",false,wItemsList)
			lItemValue = guiCreateLabel(159,22,140,19,"Adet:",false,wItemsList)
			...
			local colName = guiGridListAddColumn(gridItems, "Eşya İsmi", 0.3)
