...
wWait = guiCreateButton( ( sx - 200 ) / 2, ( sy - 60 ) / 2, 200, 60, "Lütfen bekleyiniz...", false )
...
--[[function clickItem(button, state, absX, absY, x, y, z, element)
	if (button == "right") and (state=="down") then
		...
		if element and (getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("map-system")) or getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("interiors"))) then
			element = nil
		end
...
function showItemMenu()
	...
	addEventHandler("onClientGUIClick", bYTButton, function() 
		...
		if isYoutubeURL(url) then
			triggerServerEvent("changeGhettoblasterTrack", getLocalPlayer(), item, url)
		end
	end, false)
...
function pickupItem(button, state, item)
	if (button=="left") then
		...
		if (restrain) and (restrain==1) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Eşyayı yerden alma başarısız", "Kelepçelisiniz.", 255, 0, 0)
		elseif getElementData(item, "itemID") > 0 and not hasSpaceForItem(getLocalPlayer(), getElementData(item, "itemID"), getElementData(item, "itemValue")) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Eşyayı yerden alma başarısız", "Envanteriniz dolu. "..("%.2f/%.2f" ):format( getCarriedWeight( localPlayer ), getMaxWeight( localPlayer ) ).." kg(s)." , 255, 0, 0)
...
function copyClipboard()
	...
	if setClipboard(text) then
		exports.hud:sendBottomNotification(getLocalPlayer(), "Kopyalama", "'" .. text .. "' panoya kopyalandı.")
	else
		exports.hud:sendBottomNotification(getLocalPlayer(), "Kopyalama", "'" .. text .. "' panoya kopyalanamadı.")
	end
end
...
function showInventory(player)
	if not (wItems) then
		...
		if player ~= getLocalPlayer() then
			...
			title = getPlayerName(player).." adlı kişinin "..title.."i"
		end
		...
		-- GENERAL
		lDescription = guiCreateLabel(0.025, 0.87, 0.95, 0.1, "Açıklamayı görmek için bir öğeye tıklayın.", true, wItems)
	...
	if (button=="left") then
		if (guiGetSelectedTab(tabPanel)==tabItems or guiGetSelectedTab(tabPanel)==tabKeys) then -- ITEMS
			...
			if (row==-1) or (col==-1) then
				guiSetText(lDescription, "Açıklamayı görmek için bir öğeye tıklayın.")
			else
				...
				if not items[slot] then
					guiSetText(lDescription, "Boş bir alan.")
				else
...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==7) then -- phonebook
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use /phonebook to use this item.", 255, 194, 14)
			return
		...
		elseif (itemID==30) or (itemID==31) or (itemID==32) or (itemID==33) then
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use the chemistry set purchasable from 24/7 to use this item.", 255, 0, 0)
			return
		...
		elseif (itemID==53) then -- Breathalizer
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Use /breathtest to use this item.")
			return
		...
		elseif (itemID==61) then -- Emergency Light Becon
			exports.hud:sendBottomNotification(getLocalPlayer(), "Item Usage", "Put it in your car inventory and press 'P' to toggle it.")
			return
		...
		elseif (itemID==71) then -- Notebook
			exports.hud:sendBottomNotification(getLocalPlayer(), itemName, "Use /writenote [text] to write a note. There are " .. itemValue .. " pages left.")
			return
		elseif (itemID==72) then -- Note
			exports.hud:sendBottomNotification(getLocalPlayer(), itemName, "The Note reads: " .. itemValue)
		...
		elseif itemID == 121 then
			exports.hud:sendBottomNotification(getLocalPlayer(), itemName, "A heavy box full of supplies!")
			return
		elseif (itemID==147) then -- Picture Frame --  Adams
			if(itemValue and itemValue ~= 1) then
				local split = exports.global:explode(";", itemValue)
				if(#split > 1) then
					exports.hud:sendBottomNotification(getLocalPlayer(), itemName, "Drop this in an interior to replace the texture '"..tostring(split[2]).."'.")
				else
					local url = itemValue
		...
		elseif (itemID==150) then -- ATM card
			...
			if tonumber( itemExploded[2] ) > 0 then
				exports.hud:sendBottomNotification(getLocalPlayer(), g_items[itemID][1] , "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'. Drag & Drop to a nearby ATM Machine.")
			else
				exports.hud:sendBottomNotification(getLocalPlayer(), g_items[itemID][1] , "Card Number: '"..itemExploded[1].."', Owner: '"..tostring(exports.cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'. Drag & Drop to a nearby ATM Machine.")
			end
			return
		elseif (itemID==152) then -- ID card
			local itemExploded = explode(";", itemValue )
			local text = "Adı ve Soyadı '"..itemExploded[1]:gsub("_", " ").."', Cinsiyeti: '"..itemExploded[2].."', Doğum Tarihi: '"..itemExploded[3].."', Parmak Izi: '"..itemExploded[4].."' "
		...
		elseif (itemID==212) then -- Snow Tire
			outputChatBox("[!] Karlı havalarda aracınızdan daha iyi performans almak için aracınıza koyun.", 0, 125, 255, true)
			return
		end
		...
		if itemSlot == -100 then
			exports.hud:sendBottomNotification(getLocalPlayer(), itemName, "You wear Body Armor.")
		else
