...
function updateLocalGuns(thePlayer, items)
		...
		if (itemCheck[1] == 115) then -- Weapon
			...
			if firstClipBullets == 0 then
				...
				if not (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], true, false, false)
				end
			else
				if (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], false, false, false)
				end
			end
	...
	for i, wepID in ipairs(getPedWeapons(thePlayer)) do
		if (getElementData(thePlayer, "account:id") == 2 and getElementData(thePlayer, "enableGunAttach")) then
...
function reloadA(thePlayer,weapon,ammocalc)
	...
	exports.anticheat:changeProtectedElementDataEx(thePlayer, "reloading", true, false, false)
	setTimer(checkFalling, 100, 10, thePlayer)
	exports.anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..weapon, false, false, false)
...
function useAmmo(client, weapon,wasted)
	...
	local wep = getPedWeapon(client) -- lastKnownGun[thePlayer] --
			...
			if not (ammoinclip >= clipSizeCalc) then -- Not reload if their clip is full
				if not getElementData(thePlayer, "deagle:reload") and not getElementData(thePlayer, "scoreboard:reload") then
					...
					if secondClip then
						...
						outputChatBox("Merminiz yok!", thePlayer)
					end
				end				
			end
		end
...
function checkFalling(thePlayer)
	...
	if not (isPedOnGround(thePlayer)) and (reloading) then
		-- reset state
		exports.anticheat:changeProtectedElementDataEx(thePlayer, "reloading.timer", false, false)
		exports.global:removeAnimation(thePlayer)
		exports.anticheat:changeProtectedElementDataEx(thePlayer, "reloading", false, false)
...
function giveFakeBullet(weapon, ammo)
	...
	exports.anticheat:changeProtectedElementDataEx(source, "cf:"..weapon, true, false, false)
end
