...
function clickSafe(button, state, absX, absY, wx, wy, wz, element)
	...
	if element and getElementType( element ) == "object" and button == "right" and state == "down" and getElementModel(element) == 2332 then
		...
		if getDistanceBetweenPoints3D(x, y, z, wx, wy, wz) <= 3 then
			...
			if ( dimension < 19000 and ( hasItem(getLocalPlayer(), 5, dimension) or hasItem(getLocalPlayer(), 4, dimension) ) ) or ( dimension >= 20000 and hasItem(getLocalPlayer(), 3, dimension-20000) ) or ((getElementData(getLocalPlayer(),"admin_level") >= 6) and (getElementData(getLocalPlayer(),"duty_admin") == 1 )) then
				showCursor(true)
				ax = absX
				ay = absY
				safe = element
				...
				outputChatBox("[!] #ffffffKasayı açmaya yetkiniz yok.", 255, 0, 0, true)
			end
		end
	end
end
...
function showSafeMenu()
	...
	bCloseMenu = guiCreateButton(0.05, 0.63, 0.87, 0.2, "Menüyü Kapat", true, wRightClick)
