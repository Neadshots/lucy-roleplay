	...
	function( )
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
				...
				if clickWorldItem then
					...
					if not isCursorOverInventory then
						...
						if not col or getDistanceBetweenPoints3D( x, y, z, getElementPosition( localPlayer ) ) >= 10 or isWatchingTV() then
							color = background_error_color
						elseif hoverElement then
							local ispet = getElementData(hoverElement,"type") == "pet"
							if getElementType( hoverElement ) == "vehicle" then
								color = background_movetoelement_color
							elseif getElementModel( hoverElement ) == 2203 and item [ 1 ] == 337 then -- irp-plant / OzulusTR
								color = background_movetoelement_color
							elseif ispet and getElementModel(hoverElement) == 178 and (item[1] == 339 or item[1] == 341) then -- KEDİ MAMASI VE SUYU
								color = background_movetoelement_color
							elseif ispet and getElementModel(hoverElement) ~= 178 and (item[1] == 340 or item[1] == 341) then -- KÖPEK MAMASI VE SUYU
								color = background_movetoelement_color
							elseif getElementType( hoverElement ) == "player" then
								color = item[1] < 0 and background_error_color or background_movetoelement_color
							elseif getElementModel( hoverElement ) == 2332 then -- safe
			...
			elseif not tooltipYet and getPedOccupiedVehicle(localPlayer) then
				...
				element = getHoverElement("vehicle") or getHoverElement("admin-system")
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['vehicle']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SPLIT then
								...
								elseif itemID == 115 then
									if exports.integration:isPlayerTrialAdmin(localPlayer) then -- restrict this to trial admin+ / maxime
										renameWeapon(item)
									else
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.integration:isPlayerAdmin(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.integration:isPlayerAdmin(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
