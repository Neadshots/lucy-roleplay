...
function getItemName(id, value, metadata)
	...
	if metadata.item_name == "" then metadata.item_name = false end
	...
	elseif ( id == 4 or id == 5 ) and value then
		if metadata.key_name then
			...
			local pickup = exports["interiors"]:findParent( nil, value )
	...
	if i then
		...
		elseif id == 115 then
			...
			if values[2] and tonumber(values[2]) then
				return values[2]..' ammo loaded.'
			else
				return 'N/A ammo loaded.'
			end
		-- ammopack
		elseif id == 116 then
			local values = explode(':', value)
		...
		elseif id == 273 then --Generic fish
			local FishValue = explode(":", value)
			return "Weighs a whopping: 1 lbs"
		else
			return desc:gsub("#v",value)
		end
	end
