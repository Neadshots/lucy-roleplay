...
function issuePilotCertificate(thePlayer, commandName, targetPlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer, "faction") == 15 and getElementData(thePlayer, "factionleader") == 1 then
		if not (targetPlayer) then
...
function changeLock(thePlayer)
	if exports.integration:isPlayerDeveloper(thePlayer) then
		...
		if theVehicle then
			...
			if dbid > 0 then
				exports.logs:logMessage( "[VEHICLE] " .. getPlayerName( thePlayer ) .. " changed the lock for Vehicle #" .. dbid .. " (" .. exports.global:getVehicleName(theVehicle).." - "..getVehicleName( theVehicle ) .. ")", 16)
			...
			local dbid, entrance, exit, interiortype = exports['interiors']:findProperty( thePlayer )
			if dbid > 0 then
