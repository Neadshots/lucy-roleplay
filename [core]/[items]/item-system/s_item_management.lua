...
local function notify( element, noload )
	if subscribers[ element ] then
		--[[Async:foreach(subscribers[ element ], function(subscriber)
...
addEventHandler( "savePlayer", getRootElement(),
	function( reason )
		--if reason == "Change Character" then
			outputDebugString('Envanter silindi: '..tostring(reason),3,255,255,255)
			destroyInventory()
		--end
	end
)
...
function loadItems( element, force )
	...
	elseif force or not saveditems[element] then
		saveditems[element] = {}
		local qh = dbQuery(mysql:getConnection(),  "SELECT * FROM items WHERE type = " .. getType( element ) .. " AND owner = " .. getID( element ) .. " ORDER BY `index` ASC")
		local result, rows, err = dbPoll(qh, -1);
		if #result > 0 then
			for index, row in ipairs(result) do
				saveditems[element][#saveditems[element] + 1] = { tonumber( row.itemID ), tonumber( row.itemValue ) or row.itemValue, tonumber( row.index ), tonumber( row.protected ) }
			end
			if not subscribers[ element ] then
				subscribers[ element ] = {}
				if getElementType( element ) == "player" then
...
function giveItem( element, itemID, itemValue, itemIndex, isThisFromSplittingOrAdminCmd )
	...
	if success then
		...
		if not itemIndex then
			...
			if result then
				dbQuery(
					function(qh, thePlayer, itemID, itemValue, itemIndex, isThisFromSplittingOrAdminCmd)
						...
						if rows > 0 then
							...
							if itemID == 178 then
								...
								local bID = bInfo[3]
								if not bID then
									dbExec(mysql:getConnection(), "INSERT INTO books SET title='".. (itemValue) .."', author='Unknown', book='The begining of something great...'")
									dbQuery(
										function(qh, thePlayer, itemID, itemValue, itemIndex, isThisFromSplittingOrAdminCmd)
											local res, rows, err = dbPoll(qh, 0)
											if rows > 0 then
									...
									{thePlayer, itemID, itemValue, itemIndex, isThisFromSplittingOrAdminCmd},mysql:getConnection(), "SELECT `index` FROM books WHERE `index` = LAST_INSERT_ID()")									
								end
							end
						end
					end,
				{thePlayer, itemID, itemValue, itemIndex, isThisFromSplittingOrAdminCmd}, mysql:getConnection(), "SELECT `index` FROM items WHERE `index` = LAST_INSERT_ID()")
			end
		end
							...
							if query then
								...
								if itemID == 115 then -- guns
									...
									if isThisGunDuplicated(itemValue, target) then
										...
										outputChatBox("[!] #ffffffFazla uyanık olma Query was here :P.", target, 255, 0, 0, true)
								...
								if ( (itemID >= 31) and (itemID <= 43) ) or itemBannedByAltAltChecker[itemID] then
									...
									if not exports.global:isStaffOnDuty(to) and not exports.global:isStaffOnDuty(from) then
										if hoursPlayedFrom < 10 then
											outputChatBox("[!] #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(to).."'a transfer edebilmek için 10 saat oynaman olması gerekmektedir.", from, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
										
										if hoursPlayedTo < 10 then
											outputChatBox("[!] #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(from).." size transfer edebilmesi için 10 saat oynaması olması gerekmektedir.", to, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
									end
								end
...
function getMaxWeight(element)
	...
	elseif getElementType( element ) == "vehicle" then
		...
		elseif isTruck( element ) then
			return tonumber(exports["job-system-trucker"]:getTruckCapacity(element)) or 120
		else
			return 20
		end
...
function deleteAll( itemID, itemValue )
	if itemID then
		...
		-- remove world items
		local objects = getElementsByType('object', getResourceRootElement(getResourceFromName('item-world')))
		if objects then
				...
				function(qh)
					...
					if rows > 0 then
						for index, row in ipairs(res) do
							Async:foreach(getElementsByType("object", getResourceRootElement(getResourceFromName("item-world"))), function(value)
								if isElement( value ) then
