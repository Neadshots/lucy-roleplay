...
function loadWorldItems(res)
	...
	dbExec(exports.mysql:getConnection(), "DELETE FROM `worlditems` WHERE `protected`='0' AND `itemID` NOT IN(81, 103, 169) AND ( (DATEDIFF(NOW(), creationdate) > 30 ) OR (DATEDIFF(NOW(), creationdate) > 7 AND `itemID` = 72) ) " )
