function VergiGUI(Data)
    ...
    vergiPanelWindow = guiCreateWindow(scaleW * 426, scaleH * 205, scaleW * 513, scaleH * 359, "Lucy Roleplay - Birlik Araç Vergi Sistemi • pavlov", false)
    ...
    guiGridListAddColumn(vergiPanelGrid, "Vergi Borcu", 0.4)
	for i, data in ipairs(Data) do
		...
		guiGridListSetItemText(vergiPanelGrid, row, 3, "$" .. (data[3]), false, false)
	end
	...
	addEventHandler("onClientGUIClick", guiRoot, 
		function() 
			if source == odeBtn then
				...
				triggerServerEvent("vergi:VergiOdef", getLocalPlayer(), aracID, miktar)
...
addEvent("vergi:VergiGUIf", true)
addEventHandler("vergi:VergiGUIf", getRootElement(), VergiGUI)
