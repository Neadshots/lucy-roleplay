...
function showLibrary(vehs, thePed)
	...
	GUIEditor_Window[1] = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Custom Vehicles System - Vehicle Library [Version 2.0 | 20.03.14 | Maxime]",false)
	...
	for i = 1, #vehs do
		...
		guiGridListSetItemText(grid, row, col.price, "$"..exports.global:formatMoney(vehs[i].vehprice), false, false) 
		guiGridListSetItemText(grid, row, col.tax, "$"..exports.global:formatMoney(vehs[i].vehtax), false, false) 
	...
	if thePed and isElement(thePed) and getElementData(thePed, "carshop") then
		...
		if exports.integration:isPlayerVehicleConsultant(localPlayer) or exports.integration:isPlayerSeniorAdmin(localPlayer) then
