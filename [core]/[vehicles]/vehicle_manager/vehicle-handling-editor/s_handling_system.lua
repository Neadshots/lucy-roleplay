...
function deleteUniqueHandling(theVehicle, player)
	...
	if dbid and tonumber(dbid) then
		...
		if existed then
			...
			exports.global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(player, "account:username").." has reset unique handling for vehicle #"..dbid..".")
...
function saveHandling(theVehicle1, saveToSQL, who)
	...
	if saveToSQL == 0 then -- SAVE TO ALL INSTANCES
		...
		exports.global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(who, "account:username").." has changed handlings of all instances of vehicle shop id #"..id)
		...
		dbQuery(
			function(qh, who)
				...
				if res > 0 then
					...
					exports.global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(who, "account:username").." saved unique handlings for vehicle id #"..dbid)
				end
			end,
...
function reloadAllInstancesOf(vehicle_shop_id)
	...
	exports.global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has reloaded "..count.." instances of vehicle shop id #"..vehicle_shop_id.." in game.")
end
...
function orderVehicle(vehShopID, thePed)
	...
	exports.anticheat:changeProtectedElementDataEx(source, "carshop:grotti:orderedvehicle:"..getElementData(thePed,"carshop"), tonumber(vehShopID), true)
...
function cancelOrderVehicle(shopname)
	if shopname then
		exports.anticheat:changeProtectedElementDataEx(client, "carshop:grotti:orderedvehicle:"..shopname, false, true)
	else
	--[[
		for i, v in pairs(exports["vehicle_shop"]:getCarShops()) do
			exports.anticheat:changeProtectedElementDataEx(client, "carshop:grotti:orderedvehicle:"..(v.name), false)
		end
		]]
	end
end
