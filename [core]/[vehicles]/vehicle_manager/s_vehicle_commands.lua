...
addCommandHandler("toplamvergi", 
	function(thePlayer)
		...
		if theVehicle then
			outputChatBox("[!] #f0f0f0Aracın Toplam Vergisi: $" .. tostring(getElementData(theVehicle, "toplamvergi") or 0), thePlayer, 0, 0, 255, true)
		else
			outputChatBox("[!] #f0f0f0Herhangi bir araçta değilsiniz!", thePlayer, 255, 0, 0, true)
		end
	end
)
--MAXIME
...
function togVehReg(admin, command, target, status)
	if (exports.integration:isPlayerAdmin(admin)) then
...
function unflipCar(thePlayer, commandName, targetPlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer, "faction") == 4 or exports.integration:isPlayerSupporter(thePlayer) then
...
function flipCar(thePlayer, commandName, targetPlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer,"faction") == 4 or exports.integration:isPlayerSupporter(thePlayer) then -- SFTR, working on motorbikes etc
		if not targetPlayer or not exports.integration:isPlayerTrialAdmin(thePlayer) then
			if not (isPedInVehicle(thePlayer)) then
...
function respawnAllCivVehicles(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox(" =-=-=-=-=-=- Tüm sivil araçlar respawnlanmıştır. =-=-=-=-=-=-=", getRootElement(), 255, 194, 14)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (veh) then
						...
						outputChatBox("Your vehicle was repaired by admin " .. username:gsub("_", " ") .. ".", targetPlayer)
					else
						outputChatBox("That player is not in a vehicle.", thePlayer, 255, 0, 0)
					end
				end
...
function fuelAllVehicles(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local username = getPlayerName(thePlayer)
		for key, value in ipairs(exports.pool:getPoolElementsByType("vehicle")) do
			setElementData(value, "fuel", exports["vehicle_fuel"]:getMaxFuel(getElementModel(value)), false)
		end
				...
				if rows > 0 then
					for index, row in ipairs(res) do
						if row["deleted"] == "0" then
							...
							local theVehicle = exports["vehicle"]:loadOneVehicle(dbid, false, true)
							if theVehicle then
		...
		if not (dbid) then
			...
			if not theVehicle then
				if dbExec(mysql:getConnection(), "UPDATE `vehicles` SET `deleted`='0', `chopped`='0' WHERE `id`='" .. (dbid) .. "'") then
					call( getResourceFromName( "vehicle" ), "loadOneVehicle", dbid )
...
function setVehicleFaction(thePlayer, theCommand, vehicleID, factionID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerLeadScripter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
		...
		if not (theFaggot) then
			...
			if targetVehicle then
				...
				if isVehicleDamageProof(targetVehicle) then
					...
					exports.logs:dbLog("ac"..tostring(getElementData(thePlayer, "dbid")), 4, "veh"..vehID, " Removed vehicle damage proof ")
				else
					...
					exports.logs:dbLog("ac"..tostring(getElementData(thePlayer, "dbid")), 4, "veh"..vehID, " Enabled vehicle damage proof ")
				end
			end
		end
	end
