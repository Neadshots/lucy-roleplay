...
function createVehManagerWindow(data)
    if not exports.integration:isPlayerTrialAdmin(localPlayer) and not exports.integration:isPlayerVCTMember(localPlayer) and not exports.integration:isPlayerScripter(localPlayer) then
    ...
    -- vehList1 = vehList
    gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800, 600, "Vehicle Manager v1.2 © Maxime - OwlGaming", false)
...
function loadTab(vehs, i)
    for index, veh in ipairs(vehs) do
        ...
        guiGridListSetItemText(targetList[i], row, colName[i], exports.global:getVehicleName(veh), false, false)
...
function formatLastUsed(sec)
    if sec or not tonumber(sec) then
        ...
        return exports.datetime:formatTimeInterval(sec).." ago"
    end
end
...
function createCheckVehWindow(result, adminTitle, history, notes1)
    ...
    if result[1][21] then
        ...
        false)
        guiSetEnabled(bRemoveVeh1, exports.integration:isPlayerSeniorAdmin(localPlayer))
