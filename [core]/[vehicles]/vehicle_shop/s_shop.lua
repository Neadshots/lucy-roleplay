...
function carshop_buyVehicle(paymentMethod)
	...
	if not exports.global:canPlayerBuyVehicle(client) then
		if isOverlayDisabled then 
			...
			exports.hud:sendBottomNotification(client, "Maksimum araç limiti", "Azami araç sayısına zaten ulaştınız. Ayrıntılar için /stats")
		end
		return false
	end
	...
	if (paymentMethod == "cash") then
		if not exports.global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				...
				exports.hud:sendBottomNotification(client, "Para her zaman bir problemdir..", "Bu arkadaş için elinizde yeterli para yok..")
			end
			return false
		else
	...
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then 
				...
				exports.hud:sendBottomNotification(client, "Para her zaman bir problemdir..", "Banka hesabınızda bu arkadaş için yeterli para yok...")
			end
		else
	...
	elseif (paymentMethod == "token") then
		if exports.global:hasItem(client, 263, 1) then
			exports.global:takeItem(client, 263, 1)
		else
			return false
		end
	else
	...
	call( getResourceFromName( "item-system" ), "deleteAll", 3, smallestID )
	exports.global:giveItem( client, 3, smallestID )		

	destroyElement(source)					
	exports['vehicle']:reloadVehicle(smallestID)

	outputChatBox(exports.pool:getServerSyntax(false, "s").."Aracı başarıyla satın aldınız, belirli bir alana /park yapmayı unutmayın.", client, 255, 255, 255, true)
end
		...
		function(res, rows, err)
			for index, value in ipairs(res) do
				if value.enabled == 1 then
					if not vehicles_random[value.vehbrand] then
						vehicles_random[value.vehbrand] = {}
					end
					table.insert(vehicles_random[value.vehbrand], value )
				end
			end
			self.startWizard()
		end
...
function self.randomVehicle(shopID)
	if #vehicles_random[idsToBrands[shopID]] > 0 then
		local ran = math.random( 1, #vehicles_random[idsToBrands[shopID]] )
		return vehicles_random[idsToBrands[shopID]][ran]
	else
		return false
	end
	--[[
	if shopID and tonumber(shopID) then
		local ran = math.random( 1, #carshopVehicles[shopID])
		if carshopVehicles[shopID][ran][1] then
			return carshopVehicles[shopID][ran][1], carshopVehicles[shopID][ran].price
		else
			return false
		end
	end
