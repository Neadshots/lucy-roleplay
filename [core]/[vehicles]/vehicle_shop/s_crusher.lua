...
function resetPrice(theVehicle, matching)
	if isElement(theVehicle) and getElementType(theVehicle) == "vehicle" then
		if getElementData(theVehicle, "crushing") then
			...
			if thePlayer then
				outputChatBox("Düşündüğünde geri gel!", thePlayer, 255, 194, 14)
		...
		if theVehicle and getVehicleOccupant(theVehicle) == thePlayer then -- player is driver
			if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
				...
				if getElementData(theVehicle, "requires.vehpos") then
					outputChatBox("(( Lütfen aracınızı park edin ve geri gelin. ))", thePlayer, 255, 0, 0)
				elseif price == 0 then
					outputChatBox("Bu araç " .. exports.global:getVehicleName(theVehicle) .. " hiçbir şeye değmez.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Özel aracınızı satmak için bir yönetici ile iletişime geçin. ))", thePlayer, 255, 194, 14)
				else
					setElementData(theVehicle, "crushing", price, false)
					triggerClientEvent(thePlayer, 'crusher:show', theVehicle, price)
				end
			else
				outputChatBox("Bunun için Kayıt var mı? Üzgünüm, kardeşim, dokunamaz o zaman.", thePlayer, 255, 0, 0)
			end
		end
	end
end
	...
	if theVehicle and thePlayer then
		if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
			...
			if price and price > 0 then
				...
				local result = dbExec(mysql:getConnection(),"UPDATE `vehicles` SET `owner`='-1', `faction`='5', `enginebroke`='1', `locked`='0', `engine`='0', `lights`='0', `wheelStates`='[ [ 2, 2, 2, 2 ] ]', `panelStates`='[ [ 3, 3, 3, 3, 3, 3, 3 ] ]', `doorStates`='[ [ 4, 4, 4, 4, 4, 4 ] ]' WHERE id = '" .. (dbid) .. "'")
