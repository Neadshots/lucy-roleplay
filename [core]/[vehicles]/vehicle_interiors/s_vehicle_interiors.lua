...
function enterCockpitByKey(thePlayer, key, keyState)
	...
	if vehicle then
		...
		if(model == 577 or model == 519) then --AT-400 & Shamal
			if(getElementData(thePlayer, "duty_admin") == 1 or exports.global:hasItem(thePlayer, 3, tonumber(dbid)) or (getElementData(thePlayer, "faction") > 0 and getElementData(thePlayer, "faction") == getElementData(vehicle, "faction"))) then
				allowed = true
			end
		end
		if allowed then
