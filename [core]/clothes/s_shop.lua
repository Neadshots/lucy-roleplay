	...
	function(values)
		if canEdit(client) then
			if not values.id then
				-- new clothing stuff
				dbExec(mysql:getConnection(), "INSERT INTO clothing (skin, url, description, price) VALUES (" .. tonumber(values.skin) .. ", '" .. exports.mysql:escape_string(values.url) .. "', '" .. exports.mysql:escape_string(values.description) .. "', " .. tonumber(values.price) .. ")")
				dbQuery(
					function(qh, client)
				...
				if existing then
					if dbExec(mysql:getConnection(), 'UPDATE clothing SET skin = ' .. tonumber(values.skin) .. ', description = "' .. exports.mysql:escape_string(values.description) .. '", price = ' .. tonumber(values.price) .. ' WHERE id = ' .. tonumber(values.id)) then
						--url = "' .. exports.mysql:escape_string(values.url) .. '",
