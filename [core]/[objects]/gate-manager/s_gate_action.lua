...
function canPlayerControlGate(theGate, thePlayer, password)
	...
	elseif gateProtection == 9 then --If player has access to the given vehicle
		...
		for _, vehID in ipairs(tempAccess) do
			...
			if veh then
				if(exports.global:isAdminOnDuty(thePlayer) or exports.global:hasItem(thePlayer, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and isPlayerInFaction(thePlayer, getElementData(veh, "faction"))) ) then
					return true
				end
			end
		end		
