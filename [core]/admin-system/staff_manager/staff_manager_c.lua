...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ MAXIME
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700+xExtend, 461+yExtend, "Lucy Roleplay - Yetkili Yönetimi", false)
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" then
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist[1], row, GUIEditor.gridcol.date, changelogs[i].date or "N/A", false, false)
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" and #info > 0 then
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.date, changelogs[i].date or "N/A", false, false)
				...
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.username, changelogs[i].username or "N/A", false, false)
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.from, staffTitles[tonumber(changelogs[i].team)][tonumber(changelogs[i].from_rank)] or "N/A", false, false)
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.to, staffTitles[tonumber(changelogs[i].team)][tonumber(changelogs[i].to_rank)] or "N/A", false, false)
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.by, changelogs[i].by or "N/A" , false, false)
...
function editStaff()
	...
	for i = 1, #currentPos do
		--outputDebugString(currentPos[i].." - "..(futurePos[i] and futurePos[i] or "nil"))
