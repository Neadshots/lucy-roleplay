...
function reconPlayer(commandName, targetPlayer)
	...
	if getElementData(localPlayer, "loggedin") == 1 and exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerSupporter(localPlayer) then
		--outputDebugString("[RECON] Client / Ran.")
		local reconx = getElementData(localPlayer, "reconx")
		if not (targetPlayer) then
			if not reconx then
				return outputChatBox("|| Arya Roleplay || /" .. commandName .. " [Player Partial Nick]", 255, 194, 14)
			end
			...
			if getElementData(targetPlayer, "account:username") == "LordArsen" then
				outputChatBox("Kurucuyu izleyemezsin!", 255, 0, 0)
				return false
			end
			...
			if toggleRecon(true, targetPlayer) then
				outputChatBox(" " .. targetPlayerName .. " Isimli oyuncu suanda izleniyor.", 0, 255, 0)
...
addCommandHandler("aryarecontr", reconPlayer)
...
addEventHandler( "onClientKey", root, function(button,press) 
	if getElementData(localPlayer, "reconx") and press then
	    if button == "arrow_l" or button == "arrow_r" then
	    	...
	    	if target then
	    		--outputDebugString("[RECON] Client / Swapped.")
