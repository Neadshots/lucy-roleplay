			...
			if (targetPlayer) then
				...
				if commandName == "sjail" then
					if tonumber(minutes) then
						exports.global:sendMessageToAdmins("[Sessiz Ceza]: " .. adminTitle .. " İsimli Admin " .. targetPlayerName .. " İsimli oyuncuyu " .. minutes .. " dakika cezalandırdı.")
						exports.global:sendMessageToAdmins("[Sessiz Ceza]: Sebep: " .. reason)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (who) or not (minutes) or not (...) or (minutes<1) then
			...
			if commandName == "sojail" then
				exports.global:sendMessageToAdmins("[Sessiz-Ceza]: " .. adminTitle .. " İsimli Admin " .. accountUsername .. " İsimli oyuncuyu " .. minutes .. " dakika cezalandırdı.")
				exports.global:sendMessageToAdmins("[Sessiz-Ceza]: Sebep: " .. reason)
			else
		...
		if not (who) then
			...
			if (targetPlayer) then
				...
				if not (jailed) then
					...
					exports.global:sendMessageToAdmins("[ADMIN-CEZA]: " .. targetPlayerName .. " İsimli oyuncunun "..adminTitle.." tarafından cezası sonlandırıldı.")
		...
		for key, value in ipairs(players) do
			if getElementData(value, "adminjailed") then
				if tonumber(getElementData(value, "jailtime")) then
					...
					outputChatBox("[Ceza] " .. getPlayerName(value) .. ", jailed by " .. tostring(getElementData(value, "jailadmin")) .. ", permanently,", thePlayer, 255, 194, 15)
					outputChatBox("[Ceza] Sebep: " .. tostring(getElementData(value, "jailreason")), thePlayer, 255, 194, 15)
				end
				count = count + 1
