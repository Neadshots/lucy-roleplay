	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (who) or not (minutes) or not (...) or (minutes<1) then
			...
			if (targetPlayer) then
				...
				local res = dbExec(mysql:getConnection(),'INSERT INTO adminhistory (user_char, user, admin_char, admin, hiddenadmin, action, duration, reason) VALUES ("' .. (getPlayerName(targetPlayer)) .. '",' .. (tostring(getElementData(targetPlayer, "account:id") or 0)) .. ',"' .. (getPlayerName(thePlayer)) .. '",' .. (tostring(getElementData(thePlayer, "account:id") or 0)) .. ',' .. (hiddenAdmin) .. ',0,"' .. (( minutes == 999 and 0 or minutes )) .. '","' .. (reason) .. '")' )
			...
			dbQuery(
				function(qh)
					...
					if rows > 0 then
						...
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						local res = dbExec(mysql:getConnection(),"INSERT INTO adminhistory (user_char, user, admin_char, admin, hiddenadmin, action, duration, reason) VALUES ('N/A','"..accountID.."','"..(getPlayerName(thePlayer)).."', '"..(tostring(getElementData(thePlayer, "account:id") or 0)).."','"..(hiddenAdmin).."', '0', '"..(( minutes == 999 and 0 or minutes )).."', '"..(reason).."')")
