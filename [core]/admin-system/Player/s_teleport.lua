		...
		if commandName:lower() == "goto" then
			if not (target) then
				...
				if targetPlayer then
					...
					if (logged==0) then
						outputChatBox(exports.pool:getServerSyntax(false, "w").."Kullanıcı giriş yapmadığı için işlem gerçekleştirilemedi.", thePlayer, 255, 0 , 0, true)
					else
						...
						outputChatBox(exports.pool:getServerSyntax(false, "s")..targetPlayerName.." adlı karaktere başarıyla ışınlandın.", thePlayer, 255, 255, 255, true)
					
						triggerEvent ( "frames:loadInteriorTextures", thePlayer, dimension ) -- Adams
						
						outputChatBox(exports.pool:getServerSyntax(false, "s")..getPlayerName(thePlayer):gsub("_", " ").." adlı yetkili sizin yanınıza ışınlandı.", targetPlayer, 255, 255, 255, true)
		...
		if not target then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					outputChatBox(exports.pool:getServerSyntax(false, "s")..targetPlayerName.." başarıyla çekildi.", thePlayer, 255, 255, 255, true)
					
					triggerEvent ( "frames:loadInteriorTextures", targetPlayer, dimension ) -- Adams
				
					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					if hiddenAdmin == 0 then
						outputChatBox(exports.pool:getServerSyntax(false, "s")..getPlayerName(thePlayer):gsub("_", " ").." adlı yetkili tarafından çekildin.", thePlayer, 255, 255, 255, true)
					end
