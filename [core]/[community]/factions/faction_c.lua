...
addEventHandler("onClientResourceStart", resourceRoot,
	function()
		triggerServerEvent("loadFactionsWithMyPC", localPlayer, localPlayer)--Jesse	
	end
)

addEventHandler("onClientKey", root,
	function(b, s)
		if s and b == "F3" and getElementData(localPlayer, "loggedin") == 1 and not gFactionWindow then
			showFactionMenu()
			showCursor(true)
		elseif s and b == "F3" and getElementData(localPlayer, "loggedin") == 1 and gFactionWindow then
			hideFactionMenu()
		end
	end
)
...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikLevel, birlikOnay)
	...
	if not motd then
		guiSetEnabled(gFactionWindow, false)
        ...
        if tonumber(getElementData(localPlayer, "faction")) > 0 then
       		triggerServerEvent("loadFactionsCache", localPlayer, localPlayer, getElementData(localPlayer, "faction"))
       	else
       		guiSetText(loadingLabel, "Herhangi bir birliğiniz bulunmuyor!")
       	end
	else
		guiSetEnabled(gFactionWindow, true)
		...
		guiSetText(gFactionWindow, teamName)
		...
		colName = guiGridListAddColumn(gMemberGrid, "Isim", 0.20)
		colRank = guiGridListAddColumn(gMemberGrid, "Rutbe", 0.20)
		colOnline = guiGridListAddColumn(gMemberGrid, "Statü", 0.115)
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Son Giris", 0.13)
		...
		if factionPackages and factionType >= 2 then
			colDuty = guiGridListAddColumn(gMemberGrid, "Gorevde", 0.06)
		end
		...
		for k, v in ipairs(memberUsernames) do
			...
			if (not memberLastLogin[k]) then
				...
				elseif (memberLastLogin[k]==1) then
					login = tostring(memberLastLogin[k]) .. " gun once"
				else
					login = tostring(memberLastLogin[k]) .. " gun once"
				end
			end
			...
			if (memberOnline[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colOnline, "Inaktif", false, false)
			...
			if (memberLeaders[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colLeader, "Uye", false, false)
			end
			...
			if colDuty then
				if memberOnDuty[k] then
					guiGridListSetItemText(gMemberGrid, row, colDuty, "Gorevde", false, false)
					guiGridListSetItemColor(gMemberGrid, row, colDuty, 0, 255, 0)
				else
					guiGridListSetItemText(gMemberGrid, row, colDuty, "Gorevde Degil", false, false)
		...
		guiSetText(gFactionWindow, tostring(teamName) .. " (" .. (counterOnline+counterOffline) .. " kisiden " .. counterOnline .. " Kisi Aktif)")
		
		-- Birlik SEVIYE Sistemi
		birlikSeviyesiLbl = guiCreateLabel(0.83, 0.02, 0.16, 0.05, "Birlik Seviyesi: " .. birlikLevel, true, tabOverview)
		...
		birlikSeviyesiLbl = guiCreateLabel(0.83, 0.07, 0.16, 0.05, "Birlik Onayı: " .. birlikOnayStr, true, tabOverview)
		guiLabelSetVerticalAlign(birlikSeviyesiLbl, "center")
		
		-- Make the buttons
		if (localPlayerIsLeader) then
			gButtonKick = guiCreateButton(0.825, 0.076 + 0.05, 0.16, 0.06, "Kişiyi At", true, tabOverview)
			gButtonLeader = guiCreateButton(0.825, 0.1526 + 0.05, 0.16, 0.06, "Liderlik Ver", true, tabOverview)
			gButtonPromote = guiCreateButton(0.825, 0.2292 + 0.05, 0.16, 0.06, "Terfii et/Rütbe düşür", true, tabOverview)
			
			if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) or (factionType==7) then -- Added Mechanic type \ Adams
				gButtonEditRanks = guiCreateButton(0.825, 0.3058 + 0.05, 0.16, 0.06, "Rütbe ve Maaşları Düzenle", true, tabOverview)
			else
				gButtonEditRanks = guiCreateButton(0.825, 0.3058 + 0.05, 0.16, 0.06, "Rütbeleri Düzenle", true, tabOverview)
			end
			
			gButtonEditMOTD = guiCreateButton(0.825, 0.3824 + 0.05, 0.16, 0.06, "Birlik Mesajini Duzenle", true, tabOverview)
			gButtonInvite = guiCreateButton(0.825, 0.459 + 0.05, 0.16, 0.06, "Kisi Davet Et", true, tabOverview)
			gButtonRespawnui = guiCreateButton(0.825, 0.5356 + 0.05, 0.16, 0.06, "Birlik Araclarini Respawnla", true, tabOverview)

			local _y = 0.6122 + 0.05
			if phone then
			...
			if factionPackages and factionType >= 2 then
				gButtonPerk = guiCreateButton(0.825, _y, 0.16, 0.06, "Dutyleri Duzenle", true, tabOverview)
			...
			tabVehicles = guiCreateTab("(Lider) Araclar", tabs)
			...
			gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Araci Respawnla", true, tabVehicles)
			gButtonAllVehRespawn = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Tum Araclari Respawnla", true, tabVehicles)
			...
			tabNote = guiCreateTab("(Lider) Notlari", tabs)
			...
			-- towstats
			if towstats then
				tabTowstats = guiCreateTab("(Lider) Cekici Bilgileri", tabs)
				gTowGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabTowstats)

				local totals = {[0] = 0, [-1] = 0, [-2] = 0, [-3] = 0, [-4] = 0}
				local colName = guiGridListAddColumn(gTowGrid, 'Isim', 0.2)
				local colRank = guiGridListAddColumn(gTowGrid, 'Rutbe', 0.2)
				local cols = {
					[0] = guiGridListAddColumn(gTowGrid, 'Bu hafta', 0.1),
					[-1] = guiGridListAddColumn(gTowGrid, 'Gecen Hafta', 0.1),
			...
			tabFNote = guiCreateTab("Notlar", tabs)
			...
			if factionType >= 2 then
				tabDuty = guiCreateTab("(Lider) Duty Ayarlari", tabs)
			...
			gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Birlikten Cik", true, tabOverview)
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Menuyu Kapat", true, tabOverview)
...
function showrespawn()
	...
	showrespawnUI = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Araclari Respawnla", false)
	local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Birliginizde dahil olan tum araclar respawnlanacaktir emin misiniz?",true,showrespawnUI)
	...
	gButtonNo = guiCreateButton(0.53,0.65,0.37,0.23,"Hayir",true,showrespawnUI)
...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		local width, height = 550, 540
		local scrWidth, scrHeight = guiGetScreenSize()
		local x = scrWidth/2 - (width/2)
		local y = scrHeight/2 - (height/2)
		
		wRanks = guiCreateWindow(x, y, width, height, "Rutbeler & Maaslar", false)
		...
		for i=1, 20 do
			...
			if wages and exports["integration"]:isPlayerHeadAdmin(localPlayer) then
		...
		bRanksClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Kapat", true, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Rank isimlerinizde veya maaslarda uygunsuz karakterler bulundu Ornek; '@.;", 255, 0, 0)
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Bugünki Mesajlar", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)
			
			guiSetInputEnabled(true)
			
			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Guncelle!", true, wMOTD)
			addEventHandler("onClientGUIClick", bUpdate, sendMOTD, false)
			
			bMOTDClose= guiCreateButton(0.1, 0.775, 0.85, 0.15, "Pencereyi Kapat", true, wMOTD)
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			outputChatBox("Mesajınızda uygunsuz karakterler bulundu.", 255, 0, 0)
		else
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		--else
			...
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"" .. getTeamName(theTeam) .. " Isimli birlikten çıkmak istediğine emin misin?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		--[[else]]if (playerName~="") then
			...
			outputChatBox("* " .. playerName:gsub("_", " ") .. " isimli oyuncu '" .. tostring(theTeamName) .. "' isimli birlikten atildi.", 0, 255, 0)
			triggerServerEvent("cguiKickPlayer", getLocalPlayer(), playerName)
		else
			outputChatBox("Lutfen atılacak birisini seçin..")
		end
	end
end
