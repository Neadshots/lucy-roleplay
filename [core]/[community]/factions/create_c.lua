function birlikKurGUI()
	...
	birlikWindow = guiCreateWindow((screenW - 288) / 2, (screenH - 321) / 2, 288, 321, "LUCY RPG Birlik Sistemi", false)
...
function birlikSeviyeGUI(birlikIsmi, birlikLevel)
	local screenW, screenH = guiGetScreenSize()
	local birlikFiyat = 10000 * birlikLevel
	seviyeWindow = guiCreateWindow((screenW - 289) / 2, (screenH - 181) / 2, 289, 230, "LUCY RPG", false)
	...
	seviyeYukseltBtn = guiCreateButton(13, 132, 267, 37, "Seviye Yükselt ($" .. string.gsub(birlikFiyat, "^(-?%d+)(%d%d%d)", '%1.%2') .. ")", false, seviyeWindow)
