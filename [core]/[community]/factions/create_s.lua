	...
	if para >= 10000 then
		...
		if theTeam then
			if dbExec(mysql:getConnection(), "INSERT INTO factions SET name='" .. tostring(factionName) .. "', bankbalance='0', type='" .. (factionType) .. "', level='1'") then
					...
					function(qh, thePlayer, theTeam, factionName, factionType)
						...
						if rows > 0 then
							for index, value in ipairs(res) do
								...
								setElementData(theTeam, "max_interiors", 20, false, true) --Don't sync at all / Maxime
				...
				{thePlayer, theTeam, factionName, factionType}, connection, "SELECT * FROM factions WHERE id = LAST_INSERT_ID()")
			else
		...
		if vehFaction == -1 then
			if vehOwner == playerID then
				...
				if playerBirlik then
					...
					local query = dbExec(mysql:getConnection(), "UPDATE vehicles SET faction='" .. playerBirlik .. "' WHERE id='" .. vehID .. "'")
		...
		if playerBirlik then
			if playerBirlik == vehFaction then
				if factionLeader == 1 then
					...
					outputChatBox("[!] #f0f0f0Aracı sahibine geri verebilmek için birlik lideri olmalısınız.", thePlayer, 255, 0, 0, true)
				end
			end
		else
			outputChatBox("[!] #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
