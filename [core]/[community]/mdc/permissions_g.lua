...
function canAccess( player, access_name )
	...
	local my_active_faction = getElementData(player, "faction") or -1
	local my_perm = perm or false
	if my_perm then
...
function getAdminLevel( player )
	local my_active_faction = getCurrentFactionDuty( player ) or 0
	return getElementData( player, 'mdc_admin' ), my_active_faction
end

function getCurrentFactionDuty(element)
	local playerFaction = getElementData(element, "faction")
