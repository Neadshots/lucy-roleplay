...
mdc_permissions = {
	[1] = {--LSPD
		...
		["canSeeCalls"] = true,
		["canSeeVehicles"] = true,
		["canSeeProperties"] = true,
		["canSeeLicenses"] = true,
		["canSeePilotStuff"] = true,
	},
	[47] = {--LSIA
		["canSeeWarrants"] = true,
		["canSeeCalls"] = true,
...
function hasCanAccesFactionOverlayMDC(factionID, perm)
	local factionID = tonumber(factionID)
	if mdc_permissions[factionID] then
		if mdc_permissions[factionID][perm] then
			return mdc_permissions[factionID][perm]
		else
			return false
		end
	else
			...
			function(qh)
				...
				if rows > 0 then
					for index, value in ipairs(res) do
						mdc_groups[value.id] = {
							["name"] = value.name,
							["canSeeWarrants"] = hasCanAccesFactionOverlayMDC(value.id, "canSeeWarrants"),
							["canSeeCalls"] = hasCanAccesFactionOverlayMDC(value.id, "canSeeCalls"),
							["canSeeVehicles"] = hasCanAccesFactionOverlayMDC(value.id, "canSeeVehicles"),
							["canSeeProperties"] = hasCanAccesFactionOverlayMDC(value.id, "canSeeProperties"),
							["canSeeLicenses"] = hasCanAccesFactionOverlayMDC(value.id, "canSeeLicenses"),
							["canSeePilotStuff"] = hasCanAccesFactionOverlayMDC(value.id, "canSeePilotStuff"),
						} 
					end
					setElementData(resourceRoot, "mdc_groups", mdc_groups)
				end
			end,
		mysql:getConnection(), "SELECT * FROM `factions` WHERE `type` = '2'")
	end
)
...
function cacheOnStart()
	...
	mysql:getConnection(), "SELECT * FROM `mdc_faa_licenses`")
end
...
function main ( )
	...
	if ( warrantResult ) then
		...
		while true do
			...
			local char = nil
			local characterResult = exports.mysql:query( "SELECT `charactername` FROM `characters` WHERE `id` = '".. ( row.character ) .."'" )
			if characterResult > 0 then
			...
			local account = nil
			local accountResult = exports.mysql:query( "SELECT `user`,`organization` FROM mdc_users WHERE `id` = '".. ( row.wanted_by ) .."'" )
	...
	if ( apbResult ) then
		...
		while true do
			...
			--Fetch mdc account name from ID
			local account = nil
			local accountResult = exports.mysql:query( "SELECT `user` FROM mdc_users WHERE `id` = '".. ( row.doneby ) .."'" )
			if accountResult > 0 then
				account = exports.mysql:fetch_assoc( accountResult )
			end
...
function search( query, queryType )
	...
	elseif queryType == "Person" then --Person
		local character = nil
		local criminal = nil
		local wantedUser = nil
		local crimesRow = nil
		
		local result = exports.mysql:query( "SELECT * FROM characters WHERE `charactername` = '".. ( query:gsub( " ", "_" ) ) .."'" ) --Fetch the information from the database about our character.
		...
		if result > 0 then
			...
			if result2 > 0 then --This MDC profile has been visited before.
				...
				local query = dbExec(mysql:getConnection(), "INSERT INTO `mdc_criminals` ( `character` ) VALUES ('"..character.id.."')" ) 
				local result2 = exports.mysql:query( "SELECT * FROM `mdc_criminals` WHERE `character` = '".. character.id .."'" ) --Select what the PD already knows about this character.
				if query then
					if result2 > 0 then --This MDC profile has been visited before.
						criminal = exports.mysql:fetch_assoc( result2 )
			...
			if tonumber( criminal.wanted ) == 1 then
				local result3 = exports.mysql:fetch_assoc( "SELECT `user` FROM mdc_users WHERE `id` = '".. ( criminal.wanted_by ) .."' LIMIT 1" ) --We need to figure out the wanted by's name!
				criminal.wanted_by = "No-one"
				if result3 and result3.user ~= nil then
					criminal.wanted_by = result3.user
				end
			end
	...
	elseif queryType == "Vehicle by Plate" or queryType == "Vehicle by VIN" then --Vehicle
		local q = ""
		if queryType == "Vehicle by Plate" then
			q = "v.plate = '".. ( query ) .. "'"
		elseif queryType == "Vehicle by VIN" then
			q = "v.id = '".. ( query ) .. "'"
		else
			return false
		end
		...
		if vehicle and vehicle.id ~= nil then
			...
			if tonumber( vehicle.owner ) ~= -1 then
				local owner = nil
				local result3 = exports.mysql:query( "SELECT `charactername` FROM `characters` WHERE `id` = '".. ( vehicle.owner ).."'" )
				if result3 > 0 then
			...
			elseif tonumber( vehicle.faction ) ~= -1 then
				local owner = nil
				local result3 = exports.mysql:query( "SELECT `name` FROM `factions` WHERE `id` = '".. ( vehicle.faction ).."'" )
				if result3 > 0 then
					owner = exports.mysql:fetch_assoc( result3 )
				end
			...
			if vehElement then
				...
				vehicle.model = exports.global:getVehicleName(vehElement).." "..vehicle.model
			end
			...
			triggerClientEvent( source, resourceName .. ":display_vehicle", getRootElement(), vehicle.id, vehicle.model, vehicle.color1, vehicle.color2, vehicle.color3, vehicle.color4, vehicle.plate, vehicle.faction, vehicle.owner, vehicle.owner_type, {impounded, impounder, imps}, vehicle.stolen, crimes )
		else
			triggerClientEvent( source, resourceName .. ":search_noresult", getRootElement() )
		end
	elseif queryType == "Property by ZIP Code (( ID ))" then --Property
		local result = exports.mysql:query( "SElECT * FROM interiors WHERE `id` = '"..( query ).."'" )
		if result > 0 then
...
function add_crime( charid, charactername, crime, punishment )
	...
	local addCrime = dbExec(mysql:getConnection(), "INSERT INTO `mdc_crimes` ( `crime`, `punishment`, `character`, `officer`, `timestamp` ) VALUES ( '"..( crime ).."','"..( punishment ).."','"..charid.."','"..officer.."', '"..timestamp.."' )" )
...
function add_apb( description, person )
	...
	local query = dbExec(mysql:getConnection(), "INSERT INTO `mdc_apb` ( `person_involved`, `description`, `doneby`, `time`, `organization` ) VALUES ( '"..( person ).."','"..( description ).."','"..officer.."', '"..timestamp.."', '"..( org ).."' )" )
	if query then
		main(  )
	end
end
...
function add_pilot_event( charid, charactername, crime, punishment )
	...
	local time = getRealTime( )
	local timestamp = time.timestamp
	local query = dbExec(mysql:getConnection(), "INSERT INTO `mdc_faa_events` ( `crime`, `punishment`, `character`, `officer`, `timestamp` ) VALUES ( '"..( crime ).."','"..( punishment ).."','"..charid.."','"..officer.."', '"..timestamp.."' )" )
	if query then
		search( charactername, 0 )
	end
end
...
function add_pilot_license( charid, charactername, license, aircraft )
	...
	local query = dbExec(mysql:getConnection(), "INSERT INTO `mdc_faa_licenses` ( `license`, `value`, `character`, `officer`, `timestamp` ) VALUES ( '"..( license ).."',"..(tostring(aircraft))..",'"..charid.."','"..officer.."', '"..timestamp.."' )" )
	if query then
		if mdc_faa_licenses then
...
function remove_pilot_license( charid, charactername, license_uid, licensetext )
	local query = exports.mysql:query( "DELETE FROM `mdc_faa_licenses` WHERE `id` = '"..(tostring(license_uid)).."'" )
	if query then
		local match
		if mdc_faa_licenses then
			for k,v in ipairs(mdc_faa_licenses) do
...
function update_person( charid, charactername, dob, ethnicity, phone, occupation, address, photo )
	if tonumber( photo ) > 1 then
		...
		local qSkin = exports.mysql:query( "SELECT `skin` FROM `characters` WHERE `id` = '"..( charid ).."' " )
		if qSkin > 0 then
	...
	local qUpdate = exports.mysql:query( "UPDATE `mdc_criminals` SET `dob` = '"..dob.."', `ethnicity` = '"..ethnicity.."', `phone` = '"..phone.."', `occupation` = '"..occupation.."', `address` = '"..address.."', `photo` = '"..photo.."' WHERE `character` = '"..( charid ).."' " )
	if qUpdate then
		search( charactername, 0 )
	end
end
...
function update_details( charid, charactername, details )
	...
	local qUpdate = exports.mysql:query( "UPDATE `mdc_criminals` SET `details` = '"..details.."' WHERE `character` = '"..( charid ).."' " )
	if qUpdate then
		search( charactername, 0 )
	end
end
...
function update_pilot_details( charid, charactername, pilotDetails )
	...
	local qUpdate = exports.mysql:query( "UPDATE `mdc_criminals` SET `pilot_details` = '"..pilotDetails.."' WHERE `character` = '"..( charid ).."' " )
	if qUpdate then
		search( charactername, 0 )
	end
end
...
function update_warrant( charid, charactername, wanted, details )
	...
	local qUpdate = exports.mysql:query( "UPDATE `mdc_criminals` SET `wanted` = '"..wanted.."', `wanted_by` = '"..wanted_by.."', `wanted_details` = '"..details.."' WHERE `character` = '"..( charid ).."' " )
	if qUpdate then
		search( charactername, 0 )
	end
end
...
function system_admin( )
	...
	local query = exports.mysql:query( "SELECT * FROM `mdc_users` WHERE `organization`='"..(org).."' ORDER BY id ASC" )
	if ( query ) then
		local rows = { }
		local count = 1
...
function create_account( user, pass, level, organization )
	...
	local query = "INSERT INTO `mdc_users` ( `user`, `pass`, `level`, `organization` ) VALUES ( '"..( user ).."','"..( pass ).."','"..( level + 1 ).."','"..(organization).."' )"
...
function edit_account( id, user, pass, level )
	local query = "UPDATE `mdc_users` SET `user` = '"..( user ).."' "
		..( ( string.len( pass ) > 0 ) and ", `pass` = '"..( pass ).."' " or " " )
		..( ( level ~= -1 ) and ", `level` = '"..( level + 1 ).."' " or " " )
		.."WHERE `id` = '"..id.."' "
...
function edit_self( pass )
	...
	local query = "UPDATE `mdc_users` SET `pass` = '"..( pass ).."' WHERE `id` = '"..id.."' "
...
function updateVehicleStolen( vehicleID )
	dbExec(mysql:getConnection(), 'UPDATE `vehicles` SET `stolen` = 1 - `stolen` WHERE `id` = ' .. vehicleID )
end
