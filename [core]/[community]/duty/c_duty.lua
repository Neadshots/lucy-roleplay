function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox(exports.pool:getServerSyntax("Duty", "w").."Herhangi bir işbaşı bölgesinde bulunmuyorsun.", 255, 255, 255, true)
	end
end
...
addCommandHandler("armor",
	function()
		...
		if #availablePackages > 0 then
			local dutyLevel = getElementData(localPlayer, "duty")
			if dutyLevel or dutyLevel == 1 and getElementData(localPlayer, "faction") == 1 then
				createArmourGUI()
			end
		else
			outputChatBox(exports.pool:getServerSyntax("Duty", "w").."Herhangi bir işbaşı bölgesinde bulunmuyorsun.", 255, 255, 255, true)
...
function createArmourGUI()
	...
	window = guiCreateWindow(0, 0, 520, 200, "LSPD, Commerce HQ - LUCY RPG", false)
    ...
    _25 = guiCreateRadioButton(9, 71, 501, 19, "%25 Zırh", false, window)
    _50 = guiCreateRadioButton(9, 97, 501, 19, "%50 Zırh", false, window)
    _75 = guiCreateRadioButton(9, 121, 501, 20, "%75 Zırh", false, window)
    guiSetEnabled(_75, false)
    if time.hour >= 20 and time.hour <= 6 then
    	guiSetEnabled(_75, true)
    end
   			...
   			if b == "left" then
   				...
   				elseif source == okButton then
   					if guiRadioButtonGetSelected(_25) then
						...
						triggerServerEvent("duty.setArmor", localPlayer, localPlayer, 25)
   						outputChatBox(exports.pool:getServerSyntax(false, "s").."Zırh başarıyla edinildi.", 255, 255, 255, true)
   						destroyElement(window)
   						return
   					end
   					if guiRadioButtonGetSelected(_50) then
						   --setPedArmor(localPlayer, 50)
