...
function startBackup(thePlayer, cmd) 
	...
	--if getPedOccupiedVehicle(thePlayer) then
		...
		if veh then
			...
			if veh and vehiclesDashboardRadio[model] then
				exports.global:sendLocalMeAction(thePlayer, "flicks a switch on their dashboard radio.")
				else
				exports.global:sendLocalMeAction(thePlayer, "flicks a switch on their radio.")
				end
			else
			exports.global:sendLocalMeAction(thePlayer, "flicks a switch on their radio.")
		end
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						...
						if (duty > 0) then
							...
							if cmd == "destek" then
								outputChatBox(exports.pool:getServerSyntax(factionShortName, "s")..getPlayerName(thePlayer):gsub("_", " ").." destek talebini geri çekti.", theArrayPlayer, 255, 194, 14, true)
							else
								outputChatBox(exports.pool:getServerSyntax(factionShortName, "s")..getPlayerName(thePlayer):gsub("_", " ").." artık takip etmiyor.", theArrayPlayer, 255, 194, 14, true)
							end
						end
					end	
				end
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						...
						if (duty > 0) then
							...
							if cmd == "destek" then
								outputChatBox(exports.pool:getServerSyntax(factionShortName, "s")..getPlayerName(thePlayer):gsub("_", " ").." destek talebi oluşturdu, talep radarda işaretlendi!", theArrayPlayer, 255, 194, 14, true)
							else
								outputChatBox(exports.pool:getServerSyntax(factionShortName, "s")..getPlayerName(thePlayer):gsub("_", " ").." takip ettiğini bildiriyor.", theArrayPlayer, 255, 194, 14, true)
							end
...
addCommandHandler("destek", startBackup)
addCommandHandler("takip", startBackup)
