...
function displayPdCodes(contentFromServer)
	...
	if tonumber(getElementData(localPlayer, "faction")) ~= 1 then --if player isn't in PD then stop script here, do nothing.
		return false
	end
	...
	myWindow = guiCreateWindow( 0.25, 0.25, 0.5, 0.5, "LSPD Birim Kodları - Lucy RPG v1",true)
	local tabPanel = guiCreateTabPanel ( 0, 0.12, 1, 1, true, myWindow ) 
	
	--Response & Ten Codes
	local tabCodes = guiCreateTab( "Durum Kodları", tabPanel )
	...
	--Radio Procedures
	local tabProcedure = guiCreateTab( "Telsiz Komutları", tabPanel )
	...
	--Update the window(s)
	local tlSaveButton = guiCreateButton(0.77, 0.05, 0.1, 0.07, "Kaydet", true, myWindow) -- save button, hidden from players
	...
	local tlRefreshButton = guiCreateButton(0.65, 0.05, 0.1, 0.07, "Yenile", true, myWindow) 

	
	guiSetVisible(tlSaveButton, false)
	if getElementData(localPlayer, "factionleader") == 1 then 
