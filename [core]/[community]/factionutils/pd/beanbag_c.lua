...
function isPD()
	return getElementData(getLocalPlayer(), "faction") == 1 or getElementData(getLocalPlayer(), "faction") == 116 or getElementData(getLocalPlayer(), "faction") == 59 -- LSPD faction
end
...
function switchModex()
	if isPD() and (getPedWeapon(localPlayer)==25) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty Shotgun
		...
		elseif mode == 1 then -- lethal gun mode
			outputChatBox(exports.pool:getServerSyntax(false, "s").."Silah modu başarıyla Beanbag mod olarak ayarlandı.", 0, 255, 0, true)
