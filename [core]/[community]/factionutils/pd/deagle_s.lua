function tazerFired(x, y, z, target)
	...
	if (distance<20) then
		if (isElement(target) and getElementType(target)=="player") then
			...
			exports.global:applyAnimation(target, "ped", "FLOOR_hit_f", 0, false, true, true, true, true)
...
addCommandHandler("tazerkaldir", 
	function(thePlayer, cmd, targetPlayer)
		...
		if getElementData(theTeam, "type") == 2 or exports.integration:isPlayerAdmin(thePlayer) then
...
function removeAnimation(thePlayer)
	if (isElement(thePlayer) and getElementType(thePlayer)=="player") then
		exports.global:applyAnimation(target, "ped", "FLOOR_hit_f", -1, false, true, true, true, true)
