...
function isPDDD2()
	return getElementData(getLocalPlayer(), "faction") == 1 or getElementData(getLocalPlayer(), "faction") == 116
end
...
function switchMode()
	if isPDDD2() and (getPedWeapon(localPlayer)==24) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty deagle
		...
		if mode == 0 then -- tazer mode
			...
			outputChatBox(exports.pool:getServerSyntax(false, "s").."Silah modu başarıyla Lethal mod olarak ayarlandı.", 0, 255, 0, true)
		elseif mode == 1 and ( getElementData(team, "type") == 2 or getElementData(team, "type") == 3 ) then -- lethal mode
			outputChatBox(exports.pool:getServerSyntax(false, "s").."Silah modu başarıyla Radar mod olarak ayarlandı.", 0, 255, 0, true)
		...
		elseif mode == 2 or mode == 1 and ( getElementData(team, "type") == 2 or getElementData(team, "type") == 3 ) then -- radar gun mode
			outputChatBox(exports.pool:getServerSyntax(false, "s").."Silah modu başarıyla Tazer mod olarak ayarlandı.", 0, 255, 0, true)
		...
		if (getElementType(target)=="vehicle") then
			if (getPedWeapon(localPlayer)==24) then
				...
				if (mode==2 and getPedOccupedVehicle( localPlayer ) ~= target) then
					...
					outputChatBox(exports.global:getVehicleName(target) .. " markali aracin hizi " .. math.floor(actualspeed) .. " km/h.", 255, 194, 14)
				end
			end
		end
	end
