	...
	if fromComputer or hasMDCPermissions() then
		if login and login.window and isElement(login.window) then
			...
			if not isElement(window.window) then
				...
				login.window = guiCreateWindow( x, y, width, height, "MDC Giriş Arayüzü", false )
				...
				login.rememberCheck = guiCreateCheckBox( 10, 110, width - 20, 30, "Beni Hatırla", accountData.username ~= "Username", false, login.window )
				guiEditSetMasked( login.passBox, true )
				guiSetProperty(login.passBox, 'MaskCodepoint', '8226')		

				login.loginButton = guiCreateButton( 10, 150, ( width - 20 ) / 2, 30, "Giriş Yap", false, login.window )
				...
				login.exitButton = guiCreateButton( ( width - 20 ) / 2 + 10, 150, ( width - 20 ) / 2, 30, "Kapat", false, login.window )
				triggerEvent("hud:convertUI", localPlayer, login.window)
...
function mdc_errorWin ( text )
	...
	window.dialog.closeButton = guiCreateButton( 10, 60, width - 20, 40, "Cikis", false, window.dialog.window )
...
function mdc_confirmWin ( text )
	...
	window.confirmWin.bYes = guiCreateButton( 10, 80, 110, 40, "Evet", false, window.confirmWin.window )
	window.confirmWin.bNo = guiCreateButton( 130, 80, 110, 40, "Hayır", false, window.confirmWin.window )
