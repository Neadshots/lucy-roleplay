...
addEventHandler("onResourceStart", resourceRoot, 
    function()
        ...
        mysql:getConnection(), "SELECT * FROM jailed ORDER BY id ASC")
    end
)
...
addEventHandler("removePrisoner", resourceRoot,
    function( row, removeid, fromGUI )
            ...
            if result then
                if not fromGUI then
                    local query = dbExec(mysql:getConnection(), "UPDATE characters SET pdjail=0 WHERE id="..(charID))
                else
...
addEventHandler("addPrisoner", resourceRoot,
    function( name, cell, days, hours, charges, fine, online )
        ...
        local query = dbExec(mysql:getConnection(), "INSERT INTO jailed SET charid=(SELECT id FROM characters WHERE charactername='".. (returnWhat(name, online)) .. "'), charactername='" .. (returnWhat(name, online)) .. "', jail_time=".. (jailTime) ..", updatedBy='".. (updatedWho(client, online)) .."', charges='" .. (charges) .. "', cell='" .. (cell) .. "', fine='" .. (fine) .. "'")
        if query then
            dbExec(mysql:getConnection(), "UPDATE characters SET pdjail=1 WHERE charactername='".. (returnWhat(name, online)) .. "'")
                ...
                function(qh, client)
                    ...
                    if rows > 0 then
                        for index, row in ipairs(res) do
                            ...
                            account = JFOX_MDC
                            playerName = getPlayerName( client ):split( "_" )
                            firstLetter = string.sub( playerName[ 1 ], 1, 1 )
                            playerName = firstLetter .. playerName[ 2 ]
                            dbQuery(
                                function(qh, client)
                                    local res, rows, err = dbPoll(qh, 0)
                                    if rows > 0 then
                                        ...
                                        charID = getElementData( name, 'dbid' )
                                        ...
                                        if online then
                                            ...
                                            if tonumber(fine)>0 then -- Issue Fine
                                                ...
                                                dbExec(mysql:getConnection(), "UPDATE characters SET bankmoney=bankmoney-" .. (amount) .." WHERE charactername='".. (returnWhat(name, online)) .. "'")
                                                
                                                local tax = exports.global:getTaxAmount() -- Split between gov and PD
                                                local theTeam = getPlayerTeam(client) 
                                                exports.global:giveMoney(getTeamFromName("Government of Los Santos"), amount*tax)
                                            ...
                                            if restrainedObj then
                                                ...
                                                dbExec(mysql:getConnection(), "UPDATE characters SET cuffed = 0, restrainedby = 0, restrainedobj = 0 WHERE id = " .. (getElementData( name, "dbid" )) )
                                            end
                            ...
                            {client}, mysql:getConnection(), "SELECT `id` FROM `mdc_users` WHERE `user` = '"..playerName.."'")
                        end
                    end
                end,
            {client}, mysql:getConnection(), "SELECT * FROM jailed WHERE id=LAST_INSERT_ID()")
        else
...
addEventHandler("changePrisoner", resourceRoot,
    function( name, cell, days, hours, charges, row1, online )
        ...
        local days = tonumber(days)*24
        local jailTime = ( r.timestamp + (tonumber(hours) + days) * 60 * 60  )
        local query = dbExec(mysql:getConnection(), "UPDATE jailed SET jail_time=".. (jailTime) ..", updatedBy='".. (updatedWho(client, online)) .."', charges='" .. (charges) .. "', cell='" .. (cell) .. "' WHERE charactername='".. (returnWhat(name, online)).."'")
        if query then
            dbQuery(
                function(qh, client)
                    local res, rows, err = dbPoll(qh, 0)
            ...
            {client}, mysql:getConnection(), "SELECT * FROM jailed WHERE charactername='"..(returnWhat(name, online)).."'")
        else
            outputChatBox("No character found with that name.", client, 255, 0, 0)
        end
    end
...
function assignSkin(source)
    ...
    if skin then
        ...
        dbExec(mysql:getConnection(),  "UPDATE characters SET skin = '" .. (skin) .. "', clothingid = '" .. (skinID or 0) .. "' WHERE id = '" .. (getElementData( source, "dbid" )).."'" )
    end
end
...
function sendPrisonMsg(string)
    ...
    for _, v in ipairs(exports.pool:getPoolElementsByType("player")) do
        ...
        if getTeamFromName("Los Santos Police Department") == team or getTeamFromName("San Andreas Highway Patrol") == team then
...
function processSpeakerMessage(thePlayer, commandName, ...)
    if not (...) then
        ...
        if (getElementInterior(thePlayer) == speakerInt and speakerDimensions[getElementDimension(thePlayer)] and exports.factions:isPlayerInFaction(thePlayer, 59)) or (getDistanceBetweenPoints3D( px, py, pz, speakerOutX, speakerOutY, speakerOutZ ) < 100 and exports.factions:isPlayerInFaction(thePlayer, 59)) then
