...
function createNewStation(name, ip, donorStation)
	if name and ip then
		if donorStation then
			...
			dbExec(mysql:getConnection(),"INSERT INTO `radio_stations` SET `id`='"..smallestID.."', `station_name`='"..exports.global:toSQL(name).."', `source`='"..exports.global:toSQL(ip).."', `order`='"..smallestID.."', `owner`='"..getElementData(client, "account:id").."', `expire_date`=(NOW() + interval 30 day) ")
			...
			local smallestID = SmallestID()
			dbExec(mysql:getConnection(),"INSERT INTO `radio_stations` SET `id`='"..smallestID.."', `station_name`='"..exports.global:toSQL(name).."', `source`='"..exports.global:toSQL(ip).."', `order`='"..smallestID.."' ")
			exports.hud:sendBottomNotification(client, "Radio Station Manager", "New radio station has been successfully created! (Name: "..name..", URL: "..ip..")")
			forceUpdateClientsGUI()
		end
	else
...
function editStation(id, name, ip)
	if id and name and ip and dbExec(mysql:getConnection(),"UPDATE `radio_stations` SET `station_name`='"..exports.global:toSQL(name).."', `source`='"..exports.global:toSQL(ip).."' WHERE `id`='"..id.."' ") then
