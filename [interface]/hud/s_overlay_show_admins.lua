		...
		for k, arrayPlayer in ipairs(players) do
			...
			if logged == 1 then
				if exports.integration:isPlayerScripter(arrayPlayer) then
					...
					if (hiddenAdmin == 0 or ( exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) ) ) and not dvpUsernames[getElementData(arrayPlayer, "account:username")] then
						local r, g, b = 0, 255, 0 --hud colour
						local cR, cG, cB = 0, 200, 10 --chatbox colour
						if(hiddenAdmin == 1) then
							r, g, b = 200, 200, 200
