...
Get = {
    debug = false,
    cache = {
        ['location'] = ':infoscreen/storage/cache.xml',
        ['players'] = {},
    },
...
addEvent(sha256('Infoscreen'..md5('parsrp')..'checkifexist'),true)
addEventHandler(sha256('Infoscreen'..md5('parsrp')..'checkifexist'),root,function(username)
    
    if not username then return false end 
    if Get:_find(username) then 
        triggerClientEvent(root,sha256('Infoscreen'..md5('parsrp')..'callback'),root,true)
    else 
        triggerClientEvent(root,sha256('Infoscreen'..md5('parsrp')..'callback'),root,false)
    end

 end)

 addEvent(sha256('Infoscreen'..md5('parsrp')..'add'),true)
 addEventHandler(sha256('Infoscreen'..md5('parsrp')..'add'),root,function(username)
