...
Info = {
    ...
    _method = function(self,value)
        ...
        if (value == 'create') then
            ...
            self.cache.html = WebWindow:new(Vector2(0,0), Vector2(self.screen.x, self.screen.y), "http://mta/infoscreen/html/interface.html", true);
        ...
        elseif (value == 'destroy') then 
            ...
            setTimer(function()
                ...
                localPlayer:setData('infoscreen:state',false,false)
    ...
    index = function(self)
        if self.state then 
            ...
            if self.debug then 
                outputDebugString('[F1-INFO] "infoscreen:state" adlı kullanıcı (client) bazlı değişken değeri '..tostring(not self.state)..' olarak ayarlandı.',0,240,240,240)
            end
            ...
            localPlayer:setData('infoscreen:state',true,false)
...
--    triggerServerEvent(sha256('Infoscreen'..md5('parsrp')..'checkifexist'),root,tostring(localPlayer:getData(Info.key)))
--end)
--
--addEvent(sha256('Infoscreen'..md5('parsrp')..'callback'),true)
--addEventHandler(sha256('Infoscreen'..md5('parsrp')..'callback'),root,function(boolean) 
--    return Info:returntohtml(boolean) 
--end)
--
...
--    return triggerServerEvent(sha256('Infoscreen'..md5('parsrp')..'add'),root,tostring(localPlayer:getData(Info.key)))
end)